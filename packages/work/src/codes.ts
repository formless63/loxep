/**
 * Human/scannable identifier generation for projects.
 *
 * `projects.reference_code` (`PRJ-2026-0042`, per the design's own sketch)
 * exists for the same reason `counterparties.reference_code`,
 * `acquisitions.reference_code`, and `inventory_items.item_code` do: people
 * label things, and a UUID is not a label.
 *
 * The sequence is derived per YEAR from the rows that already exist, so a
 * fresh installation starts at `PRJ-<year>-0001`. Uniqueness is enforced by
 * the database (`unique(reference_code)`); the generator retries on collision
 * rather than assuming it won.
 */
import { WorkConflictError } from "./errors.ts";

/** `PRJ-2026-0042`. */
export function projectReferenceCode(year: number, sequence: number): string {
