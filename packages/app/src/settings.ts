 ...
 * for why). This module is the WORKER'S READER: it turns three registered
 * settings into one small resolved value the composition root and the poll
 * executor can consume, and it caches that value briefly.
 *
 * ## Why a cache, and why a short one
 *
 * Every poll wants the observation caps, and every adapter build wants the
 * rate budget. Three `application_settings` reads per poll would be three
 * pointless round trips, so the resolved value is memoized for
...
export function createMonitorSettingsReader(
  ...
  async function load(): Promise<ResolvedMonitorSettings> {
    const [defaults, caps, rateBudget] = await Promise.all([
