 ...
 *
 * Every unit test injects one of these through `createReverbAdapter({
 * fetchImpl })` so the suite exercises the real request/response path —
 * headers, body parsing, error normalization — without touching Reverb at
 * all. Mirrors `packages/integrations/etsy/test/http.ts` structurally.
 */
import type { ReverbAdapterError, ReverbFetch } from "../src/index.ts";
...
function toResponse(spec: StubResponse): Response {
  ...
  if (spec.text !== undefined) {
    ...
    if (spec.contentType !== null) {
      headers.set("content-type", spec.contentType ?? "application/hal+json; charset=UTF-8");
    }
  }
...
export function createFetchStub(
  ...
  const impl: ReverbFetch = async (url, init) => {
...
/**
 * Await a promise that must reject with a {@link ReverbAdapterError}, and
 * return the error narrowed.
 */
export async function rejection(promise: Promise<unknown>): Promise<ReverbAdapterError> {
  try {
    await promise;
  } catch (error) {
    return error as ReverbAdapterError;
  }
  throw new Error("expected the call to reject, but it resolved");
}

export const TEST_TOKEN = "loxeptestpersonalaccesstoken0000000000";
