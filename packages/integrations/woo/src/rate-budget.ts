/**
 * Per-connection API rate budget for the WooCommerce boundary: a token bucket
 * every request must acquire from before hitting the network.
 *
 * This is deliberately a LOCAL copy of the eBay adapter's budget shape rather
 * than an import. Integration packages must not depend on each other
 ...
 * both behind one structural interface if it wants to.
 *
 * Semantics:
 ...
 *   per call), `acquire` throws a `WooAdapterError` of kind `rate_limited`
 ...
 *
 * WHY A BUDGET AT ALL FOR WOO: WooCommerce core imposes no documented rate
 * limit, but a self-hosted store is one PHP-FPM pool. An unthrottled backfill
 * loop is a self-inflicted denial of service against the customer's own shop,
 * which is a worse failure than being slow. The default is deliberately
 * conservative.
 *
...
export interface WooAdapterLogger {
    ...
    async acquire(cost = 1, acquireOptions) {
      ...
      if (requiredWaitMs > maxWaitMs) {
        ...
        logger?.warn?.(
          ...
          "WooCommerce rate budget exhausted; rejecting acquisition",
        );
        throw new WooAdapterError(
          "rate_limited",
          "local WooCommerce rate budget exhausted",
          {
