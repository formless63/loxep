 ...
 * (docker/compose.dev.yml, host port 5433), mirroring packages/db/test.
 *
 * `pg` is not a direct dependency of this package, so connections go through
 * @loxep/db's `createDb`/`closeDb`.
 */
...
export async function waitFor<T>(
  ...
  { timeoutMs = 8_000, intervalMs = 50, label = "condition" } = {},
): Promise<T> {
