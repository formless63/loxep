...
- When an installation opts into redaction, buyer PII exposure in the database becomes time-bounded while replay/debugging provenance and duplicate detection survive. The shipped default remains `mode: 'keep'` as stated above.
...
- Each adapter's redaction helper is **injected from the composition root**, not imported by `@loxep/commerce` — the same boundary as the provider order pagers. That injection also bounds the sweep's scope: only object types with a bound redactor are selected for rewriting, and an order class Loxep can ingest but not yet redact is counted and logged as `unhandled` rather than dragged into a batch it would occupy forever. WooCommerce, eBay, and Medusa order object types and their adapter-owned redactors are bound today; `medusa.order` joined this path when Medusa persistence landed.
