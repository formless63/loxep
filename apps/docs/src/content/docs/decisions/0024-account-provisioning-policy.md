...
**Status:** Accepted 2026-08-15 (`loxep-x2s`), with two **PROVISIONAL** sub-decisions flagged below for owner review. Refines ADR-0007 (Better Auth with OIDC and magic links) and ADR-0016 (runtime configuration and secret storage); supersedes nothing. Does not change ADR-0017's two-role model — `admin` and `member` remain the only roles, and nothing here introduces an ACL engine, an invite table, or a per-resource permission.

Shipped in `@loxep/auth` (`provisioning-policy.ts`, `oidc-claim-roles.ts`, the `createAuth` hook wiring), `@loxep/domain`'s `authProvisioningSetting`, and the `/settings/users` surface. Operator documentation: [Managing access](../../guides/managing-access/).
...
### 2. Defaults, and the bootstrap window (**PROVISIONAL**)
...
**PROVISIONAL, owner-review (this is the question `loxep-x2s` was filed to ask).** Closed-after-bootstrap is the recommendation because `member` reads real business data and because the alternative default is unrecoverable in the direction that matters: an install that was open when it should have been closed has already handed out accounts, while an install that was closed when it should have been open costs its owner one switch. It is nevertheless a **behavior change for an upgrade in place** — an existing installation that adds a colleague next week will find the sign-in silently declined until an admin either opens the method or creates the account — and it runs against this repo's own "an absent setting must not surprise an existing install" habit (`integrationsEnabledSetting`, `orderPayloadRetentionSetting`). It is filed as provisional for that reason. The specific sub-question worth a second opinion is whether **`oidc` should default to `open`** while `magicLink` defaults to `closed`: with SSO the operator's identity provider is already the gate, and "add the user in Pocket ID, they sign in" is what every other self-hosted app does. That split was rejected for now only because one coherent default ("Loxep does not create accounts for people you did not invite") is easier to reason about than a two-speed one.
...
### 6. OIDC claim-to-role mapping, and its precedence rule (**PROVISIONAL**)
...
**PROVISIONAL:** defaulting to `'create'` — manual role edits win over the IdP unless the operator says otherwise — is the recommendation, not a settled house rule. It is the conservative reading of "never override a manual demotion", and the every-sign-in mode exists so an installation that wants the opposite can say so explicitly rather than by accident.
