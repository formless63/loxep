...
| One row per shared system, with its status | The hub's `systems` collection |
| How old each status is | The system record's own last-update time, not Loxep's clock |
