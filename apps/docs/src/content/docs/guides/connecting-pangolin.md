...
[Pangolin](https://docs.pangolin.net) is a self-hosted reverse-proxy and tunnel identity provider — the access layer in front of a self-hosted estate, including (in a typical installation) Loxep itself. This connection reads orgs, sites, resources, targets, access rules, and org domains from milestone 1 on. From milestone 4 (`loxep-acj.4`) it can also **create** a Pangolin resource, add a target, or add an access rule — three additive, non-idempotent writes, ledgered so a crash never double-creates one. From milestone 7 (`loxep-acj.7`) it can also **disable, or re-enable, one existing rule** — the reversible retirement form. Nothing in this connection can ever delete anything at Pangolin: there is no delete verb anywhere in `@loxep/integration-pangolin`, permanently. Retirement means disabling a rule, never removing it — the same call, `POST /resource/{resourceId}/rule/{ruleId}`, flips `enabled` in either direction, so a rule you disable today can be turned back on later without ever touching the value that made it what it was.
...
## Retirement (milestone 7)

Owner ruling, recorded 2026-08-15: retirement is permitted, and it means **disable, never delete** — confirmed. Loxep still never issues an HTTP `DELETE` to Pangolin, in this milestone or any other; there is no delete verb anywhere in `@loxep/integration-pangolin`, permanently.

Every declared resource's rules list, on `/infrastructure/domains/$name` and `/infrastructure/fleet/$name`, now carries a **Retire** button on each enabled rule and a **Re-enable** button on each disabled one — except a `manual`-owned rule, which never gets one: Loxep's reconciler never rewrites a human's record, retirement included. Both actions require:

- **Admin access** — the same admin-only rule every write here follows.
- **The connection's write-authorization tier at `access_affecting` or higher** — one step above the `additive` tier [Writes](#writes-milestone-4) needs, because disabling (or re-enabling) an existing rule can change who reaches a resource, not just add something new.
- **A typed confirmation** naming the resource's full domain, exactly. The dialog spells out, in the operator's own language, which rule this changes and what it does — "bypasses access control for `203.0.113.7`", "blocks `198.51.100.4`", or "routes to authentication" — before you can confirm.

Behind the dialog, a **self-lockout preflight** runs before anything is sent to Pangolin, and it refuses (never just warns) three specific situations: the rule fronts Loxep's own resource or Pangolin's own dashboard resource; the resulting rule set would leave you with no address rule and no other authentication method reaching this resource; or the retirement would remove the ONLY still-live rule referencing a dynamic-IP alias (the [add-then-retire](#dynamic-ip-named-aliases-milestone-5) rule made real — add the replacement first, always). A refusal names the reason and — for a policy-tier refusal — the exact flip that would unblock it.

**Retiring or re-enabling one rule is a job, not an instant call.** Clicking either button enqueues a background task and returns immediately; the rule's row updates (muted, with a "disabled" badge) once the job completes, along with a "last rule change: retired/re-enabled by \<you\>, on \<date\>" note for the resource as a whole — the ledger's own granularity is per resource, not per rule, so that note names the most recent change to ANY rule here, not necessarily the one you are looking at.

**Retiring an already-disabled rule (or re-enabling an already-enabled one) is a safe no-op.** Unlike a rule *create*, a rule *update* is convergent — Pangolin's own `POST` either changes the rule or leaves it exactly as it already was, so there is nothing to ledger and nothing that can double-apply.

### The old-address one-click, on the aliases page
