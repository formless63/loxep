...
**A cross-cutting follow-on to this phase is now landing milestone by milestone:** [Expense Entry and Document Intelligence Design](../../architecture/expense-entry-design/) (2026-08-15) takes the expense surface from one-screen capture to a dedicated entry page with a multi-file drag-and-drop evidence pane, makes counterparties the expense payee, adds receipt line items as a first-class record, and answers the self-hostable-OCR question this phase deferred as its [open question 3](../../architecture/flipping-lifecycle-design/#open-questions). It depends on no unshipped phase, and its OCR milestones are owner-gated. *(M1 implemented, loxep-cd3.1: migrations 0023/0024 make counterparties the expense payee, with a picker/inline-create on quick entry, "link this payee" on expense detail, and the Invoice Ninja client push widened to the full mapping table. M2 implemented, loxep-cd3.2: `/finance/expenses/new` is the real two-pane entry page, over the same `ExpensesService.create`/`ReceiptsService.attach` quick entry already uses — its payee field is still plain text pending M1's picker being mounted there.)* The remaining milestones (expense lines, OCR, lines-to-inventory) are tracked against the design document's own [milestone list](../../architecture/expense-entry-design/#milestones).
