...
Two things this phase deliberately does not close. **COGS posting from inventory depletion remains Phase 5 work** — it has since shipped there, so money spent on goods does now reach the ledger, but nothing in this phase's own scope caused that. And **a manual listing could not record its sale when this phase closed**, because `orders.connection_id` was `not null` and Phase 3 declined to create the manual-order path; that decision was the design's open question 7 — **it has since shipped**: migration `0019_manual_and_draft_listings.sql` relaxed `orders.connection_id` (PROVISIONALLY, per OQ7's resolution), and `RecordSaleForm`/`recordManualListingSale` (loxep-dgf.6) write a real manual order and deplete the linked stock unit today.
