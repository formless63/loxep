...
**The missing proxy leg is now designed** (2026-08-15): [Pangolin Integration & Chain-Provisioning Templates](../../architecture/pangolin-chain-design/) lands `@loxep/integration-pangolin`, the `ProxyProviderPort`, the `sync-proxy-resource` service, and a declarative provisioning-template engine that compiles into this phase's existing reconciler rather than beside it. It is design-only, its children are filed under `loxep-acj`, and its write-authorization model is OWNER-REVIEW-CRITICAL — Pangolin is the first provider whose writes can remove the owner's own access to the estate, including to Loxep.
