...
- Projects/jobs/sites. *(**partially implemented**: `counterparty_sites` ships with a full service — `@loxep/counterparties`'s `sites.ts` (create/update/deactivate/reactivate/list) — because sites are a Counterparties-domain table under open question 14's own package rule, and that package already exists. `projects` also ships as physical schema (hierarchy-lite, entity attribution, the billing-method consistency checks) but has **no service layer**: OQ14 maps projects to a NEW `@loxep/work` package, and new package scaffolding is orchestrator-only. No project CRUD ships yet.)*
- Time and billable work. *(`time_entries` and `billing_rates` ship as physical schema only, same `@loxep/work` gap — no time-entry recording or rate-resolution service.)*
- Materials and expenses attributed to jobs. *(`project_material_uses` schema only, same gap; the `expenses`/`expense_allocations` ALTERs this design promised are **not** made in this slice.)*
