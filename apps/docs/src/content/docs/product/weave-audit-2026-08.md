...
- **Termix active-sessions GUI panel — SHIPPED 2026-08-15 (`loxep-4ah`), see below.** This line originally overstated the case when it was first written. Two of Termix's three reads (`GET /host/db/host`, `GET /status`) are genuinely UNVERIFIED — no schema exists for them in the spec. The third, `GET /open-tabs/active-sessions`, is fully specified in the generated `openapi.json` (`Termix-SSH/Docs`, regenerated 2026-08-06), so the "UNVERIFIED" label never applied to it. Bounded live probes reached the endpoint but did not observe a representative active-session row, leaving live field presence unconfirmed. The panel ships on the strength of the fully specified OpenAPI document plus defensive parsing under the accepted product decision. See [Connecting Termix](../../guides/connecting-termix/#what-loxep-reads) for what the panel shows and [Fleet Observability Design](../../architecture/fleet-observability-design/#tailscale-and-termix-in-detail--amended-2026-08-13) for the adapter's verification notes.
