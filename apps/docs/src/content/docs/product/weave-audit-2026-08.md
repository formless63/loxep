...
**Status addendum (2026-08-18, loxep-6ea):** the remaining gaps this finding named are now wired. `/finance/overview` carries a Posting card over `PostingEngine.unpostableBacklog`/`explainFact` (backlog grouped by reason, each fact's rule-candidate explanation on demand) plus a "Post now" trigger that enqueues `accounting.post-facts` off-cycle through the same `TransactionalEnqueue` pattern `infrastructure-functions.ts` uses. A read-only `/finance/posting-rules` list mounts `PostingRulesService`'s read verbs (match criteria, target accounts, priority, status) — the "why did this post to Suspense" answer. The trial balance's drill-down landed too: every account row on `/finance/books/$id` opens `LedgerReports.accountActivity` in a dialog. Separately (same bead, audit finding A12), `/finance/books/$id` gained a Statements section rendering `createStatements`' real `incomeStatement`/`balanceSheet` — there was previously no balance sheet anywhere, and `dashboard-functions.ts`'s own hand-copied income-statement SQL is now commented to point at that service as the authority (the duplication itself is unresolved, tracked as drift risk, not a bug). Rule *authoring* (create/edit/activate/disable a posting rule) remains out of scope — the list is read-only.
...
- **Termix active-sessions GUI panel — SHIPPED 2026-08-15 (loxep-4ah), see below.** This line originally overstated the case when it was first written. Two of Termix's three reads (`GET /host/db/host`, `GET /status`) are genuinely UNVERIFIED — no schema anywhere in the spec. The third, `GET /open-tabs/active-sessions`, is fully specified in the generated `openapi.json` (`Termix-SSH/Docs`, regenerated 2026-08-06); the "UNVERIFIED" label never applied to it. The gate this finding named — a live probe confirming the shape — was run twice against the owner's real instance (2026-08-14 and 2026-08-15) and both times found zero open sessions, so field PRESENCE on a live row is still unconfirmed; the panel ships anyway on the strength of the fully-specified OpenAPI document plus defensive parsing, per an explicit 2026-08-15 owner ruling ("the more info the better — this tool is meant to be used by people that trust one another") that resolved the design's own §3.3(a) gate. See [Connecting Termix](../../guides/connecting-termix/#what-loxep-reads) for what the panel shows and [Fleet Observability Design](../../architecture/fleet-observability-design/#tailscale-and-termix-in-detail--amended-2026-08-13) for the adapter's own live-verification notes.
