...
- **Expenses live in `/finance`, not `/inventory`**, even though a reseller records most spend while standing next to a lot they just bought. The workspace map already composes billing, expenses, payments, banking, accounting, and tax there; expenses are its first tenant, not its definition. "New expense" is reachable from the command palette and the sidebar, landing directly on `/finance/expenses/new` — the one-screen quick-entry dialog this paragraph originally described is REMOVED (owner reversal, 2026-08-17; see [Expense Entry and Document Intelligence Design](../../architecture/expense-entry-design/#the-route-and-what-happens-to-quick-entry)).
