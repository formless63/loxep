...
- Columns live in a sibling `columns.tsx` as `ColumnDef<DataTableFeatures, T>[]`. `DataTableFeatures` (`@/lib/table-features`) is the one `tableFeatures()` registry every DataTable primitive and consumer shares — TanStack Table v9 resolves a table's state/API surface from an explicit feature registry rather than bundling everything, so `ColumnDef`/`Column`/`Row`/`Table` all take it as their first generic. Reference: `src/features/products/components/product-tables/columns.tsx`.
