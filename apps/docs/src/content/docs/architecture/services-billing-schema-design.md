...
**Implementation status: TWO MILESTONES of this design are implemented, PROVISIONALLY — the counterparty core, then projects/time/rates/materials/sites, the second now including its service layer.** Migration `0006_expenses_and_counterparties.sql` shipped `packages/db/src/schema/counterparties.ts` and `packages/counterparties`; migration `0011_projects_time_and_sites.sql` (`loxep-nw0`) added `packages/db/src/schema/projects.ts`, `counterparty_sites` (in `counterparties.ts`), and `packages/counterparties/src/sites.ts`. Together they create **nine of this document's nineteen tables**. `service_plans`, `subscriptions`, `service_periods`, `invoices`, `invoice_lines`, `invoice_line_sources`, `invoice_payments`, and `counterparty_identifiers` remain **design only**, because the first [OWNER-REVIEW-CRITICAL open question](#open-questions) — now answered Ninja-first, but with nothing native shipping yet — bounds how much of the billing/services model has a consumer. `projects`, `billing_rates`, `time_entries`, and `project_material_uses` shipped as physical-schema-only in the same migration; [open question 14](#open-questions)'s own recommendation mapped them to a new `@loxep/work` package that did not yet exist at that time, and new package scaffolding was an orchestrator decision. **`@loxep/work` has since been scaffolded and this document's service layer for those four tables now ships in it**: project CRUD and status lifecycle, time-entry recording with the six-scope-plus-manual-plus-unresolved rate-resolution ladder, material-use recording, and the unbilled-work read model (covering only the time-and-materials half of the design's four-source join, with the "billed" side an injectable seam — see "What shipped — Migration B" below). **No table this document does not own was altered**: `orders` gains no `counterparty_id`, and `expenses`/`expense_allocations` gain none of the columns this document plans for them. **A third, non-schema milestone has since shipped in two parts**: the Invoice Ninja adapter (`loxep-v5r.4`, `@loxep/integration-invoiceninja`) and an on-demand draft-invoice push (`loxep-v5r.5`), which now runs end to end — `apps/web/package.json` declares both `@loxep/integration-invoiceninja` and `@loxep/work` as dependencies. See [Provisional implementation decisions (partial)](#provisional-implementation-decisions-partial).
...
- **`unique nulls not distinct` requires PostgreSQL 15+**, which the `timescale/timescaledb-ha:pg18.4-ts2.29.2-all` target provides. The precedent and the portable fallback are the same as [`channel_listings`](../commerce-schema-design/#channel_listings): a unique expression index over `coalesce(...)`.
...
This section now covers **two** schema milestones (the counterparty core, migration 0006; Migration B — projects, time, rates, materials, and sites, migration 0011, `loxep-nw0`) plus a third, non-schema milestone: the Invoice Ninja adapter and its on-demand draft-invoice push (`loxep-v5r.4`, `loxep-v5r.5` — see below). It still says nothing about services, subscriptions, or service periods, because none of them is built, and `invoices`/`invoice_lines`/`invoice_line_sources` remain design-only — the push milestone works around that gap explicitly rather than building any of the three early.
...
services       packages/counterparties/src/sites.ts         (@loxep/counterparties)
               ...
               packages/work/src/                           (@loxep/work, scaffolded
                                                            and filled in after this
                                                            milestone first shipped —
                                                            see below)
tests          packages/db/test/schema-projects.test.ts    (constraint tests for all
               ...
               packages/work/test/                          (58 tests, real PostgreSQL
                                                            — see below)
```

**`@loxep/work` now exists and owns the service layer for `projects`, `billing_rates`, `time_entries`, and `project_material_uses`.** It was scaffolded via `add-domain-package` after open question 14's recommendation was confirmed (below), with only `@loxep/db` and `zod` declared as dependencies — deliberately excluding `@loxep/domain` (no audit-trail integration, unlike every other Phase 6 service package) and `@loxep/inventory` (materials recording references inventory rows by id but writes no `inventory_movements` row itself). This milestone ships:

- the four Work-domain tables exactly as this document sketches them, with every `CHECK`, FK, and index the design names;
- `packages/db`-level tests exercising those constraints directly, unchanged;
- `packages/work/src/projects.ts` — project CRUD, `updateStatus` (with `completedOn`/`closedAt` conveniences over the open `status` set), `reattributeEntity`, `list`/`listWithChildren`, and hierarchy-lite enforcement (a project may not become a grandchild). The entity-attribution ladder implements rungs 1 (`manual`), 2 (`counterparty_role_default`, resolved from `counterparty_entity_roles`), and 4 (`unattributed`) automatically; rung 3 (`installation_default`) needs a setting that lives in `@loxep/domain`, which this package does not depend on, so it is accepted only as an explicit caller-supplied value;
- `packages/work/src/billing-rates.ts` — CRUD over the rate CARD itself (`billing_rates`), since the resolution ladder has nothing to resolve without a way to write rate rows;
- `packages/work/src/rates.ts` — the six-scope-plus-manual-plus-unresolved resolution ladder exactly as this document specifies it (`project_person` → `project` → `counterparty` → `person` → `activity` → `installation` → `unresolved`, later `effective_from` wins a same-scope overlap), run independently for `rate_kind = 'bill'` and `rate_kind = 'cost'`;
- `packages/work/src/time.ts` — time-entry recording, resolving and FREEZING both rates at record time (never re-resolved by an unrelated edit; `reresolveRates` is the sole, deliberate, explicit exception), plus `timeEntryBillableAmount`/`timeEntryCostAmount` as exact `numeric(20,6)` decimal-string arithmetic (`packages/work/src/decimal.ts`, a small self-contained BigInt-based module — `@loxep/work` has no dependency on `@loxep/commerce`'s equivalent);
- `packages/work/src/materials.ts` — material-use recording, snapshotting `unit_cost_amount`/`currency` from `inventory_items` at consumption for the `inventory_basis` case (refusing a caller-supplied currency that disagrees with the item's own, since Phase 4 carries no FX) and computing a markup-derived `unit_charge_amount` when one is not given explicitly;
- `packages/work/src/unbilled.ts` — the unbilled-work read model, the design's own "most valuable single number a service business has." It can only ever compute the time-and-materials half of the design's four-source join (`service_periods` is design-only; `expenses.project_id` is one of the ALTERs this document's own migration plan defers — see [What is still design-only](#what-is-still-design-only)), and the "billed" side is an injected `BilledResolver` seam defaulting to `alwaysUnbilledResolver` with a `TODO(invoice_line_sources)` naming exactly the anti-join a future billing package's resolver should run.

`counterparty_sites` remains the one table in this domain area WITH a service in `@loxep/counterparties` rather than `@loxep/work`, because sites are a Counterparties-domain table under OQ14's own rule (see the design's "A site is owned by the counterparty, not by a project" and contradiction 7) and `@loxep/counterparties` already existed — OQ14 does not block it the way it blocked Work.

### What shipped — the Invoice Ninja adapter and on-demand push (loxep-v5r.4, loxep-v5r.5)

No new tables here, and none of `invoices`/`invoice_lines`/`invoice_line_sources` is built — this milestone works entirely within `external_resources`/`resource_links` (already shipped) plus one new provider adapter package. It is the design's "invoicing and the Invoice Ninja round-trip" milestone, and it is now real end to end for a draft push.

```text
adapter        packages/integrations/invoiceninja/     (@loxep/integration-invoiceninja,
                                                         loxep-v5r.4 — clients.ts/invoices.ts
                                                         read-write mappings, X-API-TOKEN
                                                         live-confirmed against a real
                                                         self-hosted instance)
composition    apps/web/src/server/finance-billing.ts          (pure — validation, the
               apps/web/src/server/finance-billing-functions.ts idempotency-guard target
                                                                 resolution, the
                                                                 external_resources/
                                                                 resource_links row
                                                                 shapes, and the real
                                                                 adapter/@loxep/work
                                                                 wiring; loxep-v5r.5)
ui             apps/web/src/features/finance/components/
                 push-draft-invoice-dialog.tsx           (on /finance/overview, shown only
                                                          when an invoiceninja connection
                                                          exists; hand-entered lines plus
                                                          an optional "load unbilled work"
                                                          action)
tests          apps/web/src/server/finance-billing.test.ts     (20 tests, Bun's built-in
                                                                 runner — see the honest
                                                                 gap noted below)
```

**`pushDraftInvoice({ counterpartyId, projectId?, connectionId, lines })` is real end to end.** `apps/web/package.json` now declares both `@loxep/integration-invoiceninja` and `@loxep/work` (this milestone first shipped with both undeclared and the two calls blocked, throwing a typed `FinanceBillingDependencyError` naming exactly the missing packages — that intermediate state is preserved in git history rather than described further here, since it no longer reflects reality). The idempotency guard — "does a `resource_links` row already exist with `provider='invoiceninja'`, `purpose='billing_invoice_draft'`, scoped to this project (or this counterparty with no project)" — runs first and short-circuits before any Invoice Ninja call; past it, `pushDraftInvoice` resolves the chosen connection's `invoiceninja_credentials` bundle and non-secret `config.invoiceninja.baseUrl`, builds a real `InvoiceNinjaAdapter`, reuses an existing `purpose='billing_client'` link when one exists (never re-creates a client it already pushed), and creates the client/invoice via `createClient`/`createInvoice`. Both linkage rows persist through `onConflictDoNothing` targeting `resource_links`' unique constraint, so an at-least-once retry can never double a link.

**The client/invoice `url` recorded on each `external_resources` row is a best-effort Invoice Ninja admin-UI guess (`<baseUrl>/clients/<id>`, `<baseUrl>/invoices/<id>`), not independently live-confirmed.** This routing pattern is inferred from Invoice Ninja's conventional URL shape rather than an observed write. Revisit it after an explicitly opted-in, disposable live draft push; do not encode local credential paths or deployment state in tracked documentation.

**`purpose='billing_invoice_draft'` is a new, narrower purpose than this document's own `delivery_document`.** [The Invoice Ninja round-trip](#the-invoice-ninja-round-trip) table below names `invoiceninja invoice → invoice → delivery_document` for a real, `issued`-status `invoices` row — which cannot exist while `invoices` is design-only. Reusing `delivery_document` for a bare draft now would make a future reader unable to tell a genuine issued-invoice link from a draft-only push once the real table lands, so the push milestone mints its own purpose and links to the `project` (or `counterparty`) instead of a nonexistent `invoice` row. This is recorded here rather than silently drifting from the table below; when `invoices` ships, the draft-push code path should be revisited alongside it rather than assumed compatible.

**The guard's honest limit:** it is scoped per-project (or per-counterparty), not per-source-fact. `invoice_line_sources` — the table that would let the guard check "were these EXACT `time_entry`/`project_material_use` rows already billed" — is design-only, so once a draft exists for a project, a second push for that project is refused even if new unbilled work has since accrued. That is the same honest posture `@loxep/work`'s `unbilled.ts` takes on the read side, applied to the write side.

**Line items default to hand-entry, with an optional load from `@loxep/work`.** `pushDraftInvoice`'s signature still takes `lines` explicitly — the composition stays decoupled from `@loxep/work` — but the dialog now also offers `listUnbilledWorkForBilling`, which runs `@loxep/work`'s `createUnbilledWorkService` with its own honest `alwaysUnbilledResolver` default (since `invoice_line_sources` does not exist, nothing can tell it a row is already covered by an open draft push) and appends the results as editable rows. A time entry with an unresolved bill rate loads with a blank unit cost rather than a guessed one, matching `unbilled.ts`'s "never a plausible-but-wrong number" rule.
...
- **OQ14 — the proposed domain-to-package rule, adopted twice, with two different results, and now fully applied a second time.** First application (counterparty core): `@loxep/counterparties` passes all three tests cleanly; expenses landed in `@loxep/accounting` rather than `@loxep/domain`, diverging from Phase 5's own recommendation, as this document predicted would be the first thing to test the rule against. Second application (this milestone, loxep-nw0), in two steps: the schema-only step named the answer — **`@loxep/work` for projects, time, rates, materials, services, subscriptions, and periods** — and stopped, because the package did not exist and new package scaffolding is orchestrator-only. That package has since been scaffolded (via `add-domain-package`) and filled in: project CRUD, time-entry recording with rate resolution, material-use recording, and the unbilled-work read model now live in `@loxep/work`, confined to the `@loxep/db` + `zod` dependency boundary the rule's package-boundary discipline implies (no `@loxep/domain` audit integration, no `@loxep/inventory` movement-writing — see "What shipped — Migration B" above). This is the rule doing exactly what OQ14 said it should, completed: deciding a package boundary BEFORE code goes in, and then the code going into exactly that boundary once it exists — `packages/db` (nobody's domain package), `@loxep/counterparties` (the one table this milestone's tables reference that the rule assigns elsewhere), and now `@loxep/work` itself.
...
- **No overlap-prevention constraint on `billing_rates` across effective-dated rows at the same scope**, exactly as the design recommends: an exclusion constraint over a six-shape nullable tuple is hard to write and easy to get wrong for a problem a report solves. Resolution order (which row wins) is a SERVICE concern, and `@loxep/work/src/rates.ts` now builds it: first-match-wins by declared precedence, later `effective_from` wins a same-scope tie (a same-`effective_from` tie additionally breaks on `id` for a deterministic, testable result — a narrower case the design does not specify a rule for).
...
The `@loxep/work` service layer (loxep-nw0, completing the same slice) adds these decisions, none of which change a single column or `CHECK`:

- **`time_entries.billing_rate_id` is one nullable FK; the design resolves bill and cost rates independently.** The schema does not say which resolution the single column records. Implementation reads it as the BILL resolution's `billing_rates.id` (the column's name says "billing", not "costing"); the cost resolution's rung is fully recorded in `cost_rate_source` even though its own row id is not separately stored. Flagged for review in `time.ts`'s own module doc.
- **A bill/cost currency mismatch is a refused write, not a silently-picked value.** `time_entries.currency` is one column shared by both rates. If the bill and cost ladders resolve to rates priced in different currencies — a misconfigured multi-currency rate card, not a normal installation — there is no column to hold both truths, so `record()` and `reresolveRates()` both raise `WorkValidationError` rather than losing one currency. Tested (`test/time.test.ts`'s currency-mismatch case).
- **The entity-attribution ladder implements three of the design's four rungs; `installation_default` is accepted only as an explicit value.** Rung 3 needs an installation-wide default-entity setting that lives in `@loxep/domain` (`settings-defaults.ts`), a package `@loxep/work` deliberately does not depend on. `projects.ts` resolves `manual` and `counterparty_role_default` automatically and falls back to `unattributed`; a caller that holds both packages may still pass `entityAttributionSource: 'installation_default'` explicitly, and the service records it verbatim rather than inferring it.
- **Materials recording writes no `inventory_movements` row, by construction rather than by omission.** `@loxep/work` declares no dependency on `@loxep/inventory`. `materials.ts` reads `inventory_items.landed_cost_amount`/`currency` directly through `@loxep/db` (a schema-level read, not a service call) to snapshot cost at consumption, but a `project_material_uses` row referencing an `inventory_movement_id` requires a caller holding both packages to have already written that movement (through `@loxep/inventory`) in the same transaction. Cross-domain rule 1 — Projects references Inventory's stable ids and does not mutate Inventory's canonical tables — holds by construction, not by discipline.
- **`decimal.ts` is a small, deliberately self-contained reimplementation, not an import of `@loxep/inventory`'s equivalent.** `@loxep/inventory/src/decimal.ts` is built over primitives imported from `@loxep/commerce`; `@loxep/work` has neither dependency. Its own `decimal.ts` implements exactly the exact-decimal-string arithmetic time-entry and material-use amounts need (multiplication, division-by-a-small-integer for `minutes / 60`, summation) in scaled `BigInt`, never a JS float — a third private implementation of the same underlying technique the codebase already has twice, accepted here because the alternative was adding a dependency the package's own scope statement refuses.
- **The unbilled-work read model's `BilledResolver` seam is injected, not hard-coded to `alwaysUnbilledResolver`.** `createUnbilledWorkService({ db, billedResolver? })` accepts an optional resolver so a future billing package can supply one backed by `invoice_line_sources` without `@loxep/work` ever depending on it; the default treats every fact as unbilled, which is honestly correct today since nothing could ever mark one billed. `summarize()` additionally refuses a currency-bucket total when any row in it lacks a computable amount (an unrated time entry or an unpriced material use), returning `null` and a named gap count instead — the same "an honest gap beats a plausible wrong number" posture Phase 4 and Phase 5 both took.
- **58 new tests** (`packages/work/test/`: `projects.test.ts`, `time.test.ts`, `materials.test.ts`, `unbilled.test.ts`), all against real PostgreSQL, covering CRUD round-trips, every rung of the resolution ladder individually (including the same-scope later-`effective_from`-wins tie and an expired-rate fallthrough), exact decimal-string derived amounts, the currency-mismatch refusal, the locked-row boundary, and the unbilled-work read model over a seeded fixture including the injected-resolver exclusion case.
...
`@loxep/work`'s service layer (completing loxep-nw0) is verified the same way, against the same running PostgreSQL/TimescaleDB instance rather than mocks: all 58 tests pass against real scratch databases, `bun --cwd packages/work typecheck` is clean, and `oxlint` reports zero findings against `packages/work/src` and `packages/work/test`.
...
5. **The `billing_rate_id` interpretation.** `time_entries.billing_rate_id` is read as the BILL resolution's row id, not the cost resolution's, because the schema has one column for two independent resolutions and does not say which. Cheap to revisit — it is a read-time convention, not a stored fact that would need migrating — but worth a reviewer's explicit sign-off, or a second column, if the cost rate's own row id turns out to matter somewhere.
6. **The `installation_default` entity-attribution rung is caller-supplied, not resolved.** `@loxep/work` cannot reach `@loxep/domain`'s settings without a dependency its own scope statement refuses, so a project attributed by that rung today is only ever attributed because something upstream of this package resolved the setting and passed it in explicitly. If that never happens in practice, every project without a customer-role default silently stays `unattributed` rather than picking up the installation default the design's ladder promises.
7. **The unbilled-work read model's coverage gap.** `unbilled.ts` computes only the time-and-materials half of the design's four-source join; `service_periods` and `expenses` are unreachable until later Phase 6 milestones ship. `coversSourceTypes` names this in the return shape so a caller cannot mistake a partial total for the whole picture, but the read model's headline promise — "what have we delivered and not charged for" — is currently incomplete by construction, not by bug.
