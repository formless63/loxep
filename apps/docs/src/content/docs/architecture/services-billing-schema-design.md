...
- **`unique nulls not distinct` requires PostgreSQL 15+**, which the `timescale/timescaledb:2.29.1-pg18` target provides. The precedent and the portable fallback are the same as [`channel_listings`](../commerce-schema-design/#channel_listings): a unique expression index over `coalesce(...)`.
