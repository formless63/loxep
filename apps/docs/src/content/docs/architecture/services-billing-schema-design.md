...
**The client/invoice `url` recorded on each `external_resources` row is a best-effort Invoice Ninja admin-UI guess (`<baseUrl>/clients/<id>`, `<baseUrl>/invoices/<id>`), not independently live-confirmed** — no write credential existed when the adapter was built (see its module doc), so this routing pattern is inferred from Invoice Ninja's conventional URL shape rather than observed. Worth revisiting once `~/.config/loxep/invoiceninja.env` exists and a real push has been run against a live instance.
