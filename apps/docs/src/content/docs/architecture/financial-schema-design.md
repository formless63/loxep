...
All migrations run through `loxep migrate` under the existing advisory lock (ADR-0018). Hand-written SQL is required in at least four places: the two `EXCLUDE USING gist` constraints, the deferred balance constraint trigger, the immutability triggers, and the composite foreign keys. Verify current Drizzle Kit capability at implementation time and drop to SQL rather than weakening any constraint. **Verify `btree_gist` availability in the `timescale/timescaledb-ha:pg18.4-ts2.29.1-all` image before relying on the exclusion constraints**; the fallback is documented above and is genuinely weaker.
