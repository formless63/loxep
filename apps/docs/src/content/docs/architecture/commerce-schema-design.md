...
The unique constraint requires `NULLS NOT DISTINCT` (PostgreSQL 15+; the deployment target is `timescale/timescaledb-ha:pg18.4-ts2.29.2-all`, so it is available). Without it, PostgreSQL treats each null `external_variation_id` as distinct and every re-sync of a non-variant listing inserts a duplicate. The portable fallback, if that clause is ever unavailable, is a unique expression index over `coalesce(external_variation_id, '')`. Verify the clause against current PostgreSQL behavior at implementation time.
...
6. **Sync scheduling — `monitor_targets`, target types `woo_orders` and `ebay_orders`.** No second scheduler and no `commerce_sync_cursors` table. Both cursors live under the same namespaced `config.commerceSync` key, exactly as the scheduler's own `config.adaptive` state does, because the cursor's fields are provider-neutral facts; only the `target_type` differs. The ownership question is answered in documentation: [Domain Boundaries](../domain-boundaries/#scheduling-is-shared-foundation-infrastructure) now describes the scheduling model as shared foundation infrastructure domains register target types against — also marked PROVISIONAL.
   ...
   **Registration gap — closed (loxep-itn).** `woo_orders` shipped registered in `@loxep/market`'s closed `MONITOR_TARGET_TYPES` enum and its `monitorTargetConfigSchemas` record; `ebay_orders` shipped after, because `packages/market` was outside the eBay change's (loxep-xh9.2) write fence. `claimDueTargets`, `recordPollSuccess`, and `recordPollFailure` read `target_type` as text, so claim → route → sync → cursor advance worked for `ebay_orders` from the start; what did not work was creating or editing such a row through `createMonitorService`, whose `targetType` was a closed `z.enum`. That gap is now closed: `ebay_orders` joins `woo_orders` in both lists, with the identical `commerceSync`/`adaptive` config shape (both target types share `@loxep/commerce`'s one provider-neutral `commerceSyncTargetConfigSchema`). The `packages/app` test that asserted the gap now asserts coverage instead.
...
8. **Buyer data — columns hold an identifier and a channel handle, nothing else.** `buyer_external_id` plus an optional `buyer_display_name`, where "display name" means a channel-native handle (an eBay username), **not** a legal name from a billing address. The Woo translator therefore leaves it null: Woo exposes no handle, and copying a customer's real name into a domain column would defeat the line this question draws. The full payload stays in `provider_objects`.
   ...
   **Retention is decided and owner-reviewed via [ADR-0021](../../decisions/0021-order-payload-retention/): the default is `keep` (payloads retained indefinitely — they feed the long-term CRM/customer-matching direction), with in-place redaction after a configurable window available as an opt-in; provenance rows and `order_source_links` are never automatically deleted under either mode.** No retention logic runs inside ingestion itself: the sweep is the separate daily `commerce.redact-order-payloads` job, which stamps `provider_objects.redacted_at` and never recomputes `payload_hash`, so an unchanged re-sync of a redacted order still matches its existing row and re-stores nothing. Ingestion retains one `provider_objects` row per distinct payload hash per order — an unchanged re-sync reuses the existing row rather than storing a second copy — which bounds growth but is not itself a retention policy.
...
- The Woo `refunded`/unrecognized-status re-mapping to `unknown` now lives in the adapter's `WOO_STATUS_MAP` / `WOO_UNKNOWN_STATUS_MAPPING` (loxep-xh9.7.3), matching where the eBay leg produces the same member. It previously lived in `@loxep/commerce`'s translator only because `packages/integrations/woo` was outside the implementing change's write fence; `@loxep/commerce`'s `wooOrderFactToCommerceFact` now reads `fact.fulfillmentStatus` verbatim.
...
1. **`fee_direction`** — ~~a new column and a new semantic, invented during implementation rather than during design~~ **RATIFIED by owner review 2026-08-12** with an explicit accounting interpretation: the two directions are not two kinds of the same thing — `seller_charge` rows are **fee expenses** (eBay final-value fees, and later Stripe/PayPal processor fees) and `buyer_surcharge` rows are **fee income**, and Phase 5's chart of accounts gives them separate destinations (a fee-expense account and a fee-income account), never a single netted "fees" bucket. The column stays exactly as shipped; posting rules must consume the direction when mapping to accounts. (The eBay leg, which reports both polarities on one order, was the confirming evidence.)
...
7. **The `woo_orders` / `ebay_orders` target type names** and the scheduling-ownership documentation edit. (The `ebay_orders` registration gap in `@loxep/market` closed under loxep-itn.)
...
8. **Buyer data retention before Phase 6.** *(Both halves now RESOLVED; the second half is PROVISIONAL pending owner review of the policy.)* Phase 3 normalizes only `buyer_external_id` and an optional display name, leaving names, emails, and addresses in retained provider payloads. Is that the right line for support workflows — and does it imply a retention policy on `provider_objects` for order payloads? *Recommendation was: hold the line on columns, and treat provenance retention as a separate policy decision.* Both were taken. The columns held; the separate policy decision is [ADR-0021](../../decisions/0021-order-payload-retention/), which answers the second half **yes, for this object class specifically**: order-class payloads are redacted in place after a configurable window (`commerce.order_payload_retention`, default 180 days, `mode: 'keep'` opts out), provenance rows and `order_source_links` are never automatically deleted, and `payload_hash` keeps identifying the original payload so re-sync dedup still matches after redaction. The sweep is the daily `commerce.redact-order-payloads` job, with each adapter's `redact*OrderFact` helper injected by the composition root.
