...
| 18 | `infrastructure.ip_aliases` | `Record<aliasName, { address, source: enum, hostname\|null, connectionId\|null, siteId\|null, previousAddress\|null, observedAt\|null, confirmedAt\|null, autoApply: boolean }>` | b | operator-CHOSEN keys (alias names), but each value is a rich composite with conditional fields (`hostname` only for `source: 'dns'`; `connectionId`/`siteId` only for `source: 'pangolin_site'`) and several system-written fields (`previousAddress`, `observedAt`) an operator must never hand-edit. Needs a dedicated add/edit/retire list UI (entity-form-dialog precedent), not a generic map row editor |
...
| **Auth & provisioning** | `auth.provisioning` (existing `ProvisioningCard`, unchanged), `auth.onboarding_oidc_prompt_dismissed` |
| **Infrastructure** | `infrastructure.caa_policy`, `infrastructure.gatus_push` (existing `GatusPushCard`, unchanged) |
| **Managed elsewhere** | `infrastructure.provider_write_policy` (link: *"Edit per-connection on Connections"* → `/settings/connections`), `integrations.enabled` (link: *"Edit per-provider on Integrations"* → `/settings/integrations`) — rendered as a plain link row, no form, so the operator is never shown a dead-end JSON editor for a setting that already has a real control elsewhere |

The **advanced/raw JSON escape hatch** does not disappear — it moves rather than vanishes. A collapsed `<Collapsible>` (or a separate "Advanced" tab) at the bottom of `/settings/application`, labelled plainly ("Raw settings (advanced)"), keeps today's exact `RawTable` + `SettingEditDialog` behavior for: (1) `integration.tailscale.ignored_devices` (class c, permanently), (2) any key present in `application_settings` that is **not** in the registry (the existing "raw stored rows" concept, unchanged — e.g. `@loxep/jobs`' `runtime.heartbeat`), and (3) as a documented fallback for any class (a)/(b) setting an operator wants to bulk-edit as JSON rather than through fields — the generic form is the front door, not the only door. This is the epic's own "advanced toggle" ask, applied literally: the JSON editor is demoted, not deleted.
...
3. **`loxep-8ja.3` — `/settings/application` rebuild on the renderer.** Replace the flat "Registered settings" `DataTable` with the grouped Cards from [section 3](#3-groupingnavigation-redesign-for-settingsapplication): one `SchemaSettingCard` per class (a) key, group headings, the two "managed elsewhere" link rows, the collapsed advanced-JSON section for raw rows + class (c). `GatusPushCard`/`ProvisioningCard` relocate into their group headings unchanged. Depends on `.2`.
4. **`loxep-8ja.4` — `integrations.enabled` row editor on `/settings/integrations`.** New per-provider visibility `Switch` on the integrations catalog grid, mirroring `WritePolicyCell`'s shape (one `useMutation` per row, `setConnectionWritePolicy`'s sibling for this setting). Independent of `.1`–`.3` (does not touch the generic renderer).
5. **`loxep-8ja.5` — `infrastructure.ip_aliases` CRUD surface.** A dedicated add/edit/retire list (entity-form-dialog precedent) for named IP aliases, including the conditional `hostname` (source=`dns`) / `connectionId`+`siteId` (source=`pangolin_site`) fields and the read-only system-written fields (`previousAddress`, `observedAt`). Independent of `.1`–`.4`.
6. **`loxep-8ja.6` — e2e for the renderer's shape coverage.** Playwright coverage exercising the generic form for at least one setting per field-mapping row (boolean, enum, number, nullable string, tag array), plus updating `document-line-drag.spec.ts`'s `documents.parser_id` flip to use the new `TextField` (or the raw-JSON advanced fallback, whichever the rebuilt page's DOM structure calls for) instead of the current `getByLabel('Value (JSON) *')` selector, which the page redesign will remove for that key. Depends on `.3` and `.4`.
