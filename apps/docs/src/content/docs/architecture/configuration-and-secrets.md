...
For the bootstrap OIDC provider id `oidc`, register the exact redirect URI `<LOXEP_PUBLIC_ORIGIN>/api/auth/callback/oidc` with the identity provider. Better Auth 1.7 uses the standard social-provider callback path; the older `/api/auth/oauth2/callback/oidc` path is not valid. The discovery document must expose a consistent issuer and usable JWKS because ID tokens are verified, even though Loxep deliberately stores the separate `local:oauth:oidc` account namespace to preserve existing provider-scoped identities across the 1.6 → 1.7 migration.

A populated 1.6 installation must use a stopped-workload cutover: register the new callback first, verify a restorable backup, stop all old-version processes, apply migration `0033`, and restart every process on Better Auth 1.7 before reopening traffic. Mixed 1.6/1.7 operation against the migrated schema is unsupported. The repository README and ADR-0020 carry the complete operator sequence.
