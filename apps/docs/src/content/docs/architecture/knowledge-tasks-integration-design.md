...
**IMPLEMENTED 2026-08-15 (Phase 8 milestone 7, loxep-ovj.7):** [Phase 8's open question 4](../fleet-observability-design/#open-questions) is answered YES — `POST /api/v1/hooks/fleet/:connectionId` now exists, exactly on the terms this section already committed to reusing: a per-connection bearer token checked through normalized connection/credential dependency work and a timing-safe comparison of fixed-length token digests, one `source_events` row, a projection job, and never a `notification_deliveries` row. Every verifier rejection receives the same HTTP status and body, but the design makes no end-to-end wall-clock-equivalence claim because a known credential can require version lookup and decryption work. What shipped is the RECEIVER and normalizers for Gatus, Beszel, and a generic (Databasus-class) contract — **not** a Vikunja or BookStack normalizer. Wiring either tool's webhook signature (Vikunja's HMAC-SHA256, BookStack's Slack-compatible payload) into this same receiver is a small, separate follow-up now that the shared question is settled, not blocked work anymore.
