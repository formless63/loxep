...
**OWNER REVERSAL, 2026-08-17 (see [decision 1's own note](#the-route-and-what-happens-to-quick-entry) for the full account).** The owner reversed decision 1's "both survive" call directly: the one-screen quick-entry dialog and `/finance/expenses/quick` are REMOVED, `/finance/expenses/new` is the single expense-entry path, and void-and-re-record now navigates there (`?reRecordFrom=<expenseId>`) instead of reopening a dialog. `M1`'s note above, that the payee picker "is mounted on the quick-entry dialog," and `M2`'s note that quick entry "gained its 'More options' link," are both now historical — read against a dialog that no longer exists.
...
**OWNER REVERSAL (2026-08-17).** Everything below this note described the state this design shipped M2 in, and the sentence *"Both survive, and the split is not a compromise"* two paragraphs down was wrong about the future, not just optimistic — the owner has since reversed it directly: **the one-screen quick-entry dialog and its redirect-only route (`/finance/expenses/quick`) are REMOVED.** `/finance/expenses/new` is now the SINGLE expense-entry path; every "New expense" affordance (sidebar, command palette, the expenses-table empty state, and the table's own header action) points at it, and there is no dialog to skip. Void-and-re-record, which used to reopen the dialog prefilled from the just-voided row, now navigates to the page instead — `/finance/expenses/new?reRecordFrom=<expenseId>`, which loads the voided expense and seeds the same fields the dialog used to prefill (never its evidence/receipts), with a small banner naming what is happening. The text below is left in place, per this document's own convention of recording status updates next to the original text rather than rewriting it — it is now a historical record of a decision that was made and then reversed, not a description of the current UI.
...
### v2 status note (loxep-zk5, owner directives 2026-08-17)

Everything above this note describes the layout M2 shipped in (evidence pane as a fixed `360px` right column, form dominant). The owner reviewed the shipped page and reversed the ratio, tightened the form's density, and answered two open questions the M3 line-items work had left implicit. Recorded here rather than rewritten above, per this document's own convention.

**Layout inversion.** The evidence/PDF preview is now the DOMINANT flexible pane (`minmax(0,1fr)`); the form is a **fixed compact column, `28rem`**, at desktop widths — the reverse of the ASCII diagram above. The reasoning: a receipt or PDF invoice is the thing an operator actually reads while composing an expense, and a `360px` fixed evidence column made a multi-page PDF practically unreadable at desktop widths, which was the owner's core complaint. The form's own field grids moved from `gap-6` to `gap-3` (the [density rules](../../development/frontend-standards/#density-and-mobile)), long field descriptions that exceeded one sentence moved behind `InfoButton`, and the evidence dropzone collapses to a slim "Drop or click to add more" strip once at least one attachment exists — full drag-target behavior unchanged, the reclaimed height goes to the preview. The mobile stacking/toggle mechanism (`ToggleGroup`, `hidden md:block`, both panes always mounted) is untouched — only the two desktop column WIDTHS invert, not the responsive mechanism.

**Line-item shape v2.** Each `expense_lines` row gains a `unit` column (nullable, closed set — see the connective field table below) and the line editor gains a per-line **Kind** select over the *existing* `line_kind` CHECK set (item/shipping/tax/fee/discount/other — no new column; shipping-as-a-line arrives through this, exactly as [the schema doc already anticipated](#expense_lines)). The three numeric fields — quantity, unit price, subtotal (`line_amount`) — are FILL-TWO-DERIVE-THIRD: type any two and the third computes and renders muted/italic until directly edited, at which point the operator owns it and it stops recomputing. Decimal-safe by construction (integer micro-unit `BigInt` arithmetic, division rounds half-up at 6dp, matching `expense_lines`' own `numeric(20,6)` scale) — never JS float math on a value that reaches persistence. `line_amount` stays the one number the server trusts; `unit`/`quantity`/`unit_amount` remain informational, exactly as the original schema doc states.

**Categories: the two recorded answers.** The owner asked directly whether `expenses.category` should tie back to the chart of accounts. Answered, and recorded here because it closes a question this document left open by omission:

1. **No categories table, no chart-of-accounts foreign key.** `expenses.category` stays the open `text` set it already was ([Conventions inherited](#conventions-inherited)) — a creatable combobox now sources its suggestions from the distinct categories already used in the installation plus the starter vocabulary, but free text still creates. The reasoning: the posting-rules engine (`packages/accounting/src/posting-rules.ts`, `posting_rules`/`posting_rule_lines` — shipped by migration 0010, ADR territory this design never re-litigates) **already is** the designed category → ledger-account connection. A second, parallel `category → account` mapping on `expenses` itself would be a second truth for the same fact, with no arbiter between them the moment they disagree. When a category has no matching posting rule yet, the combobox shows a calm inline hint — "posting rules map categories to accounts" — linking to the books surface (`/finance/books`; no dedicated posting-rules management page exists in `apps/web` yet, a real gap, not part of this pass).
2. **No per-line category.** Per-LINE categorization was considered and deliberately deferred: the accounting grain is the expense HEADER, not the line. A mixed-vendor receipt (some goods for resale, some genuine expense) already has its designed split — goods route to an acquisition per line via the confirm paths ([How a line reaches inventory](#how-a-line-reaches-inventory--and-the-rule-that-stops-it)), and `expense_allocations` attributes money within one expense when a split is needed. A per-line category column would duplicate work `expense_allocations.ledger_account_id`/`ledger_accounts` already does at the split level, for no user-visible gain. Revisit only if a real posting need appears that these two mechanisms cannot express.
...
**Status update (loxep-zk5, migration 0031, 2026-08-17): `unit text null` added**, with the same house rule as `line_kind` — a closed set, `CHECK`ed, TS union (`EXPENSE_LINE_UNITS`), no PG enum. `null` is the ordinary case. Carried verbatim end to end and never part of any persisted arithmetic — see [the v2 status note above](#v2-status-note-loxep-zk5-owner-directives-2026-08-17) for the line editor's fill-two-derive-third behavior that produces `quantity`/`unit_amount`/`line_amount` client-side before submit.
...
| counted in what | — | — | `unit text null` **(v2, migration 0031)** — closed set, `CHECK`ed, never arithmetic | — |
