...
This is also the first Loxep integration whose writes can **remove the operator's access to managed services**. A wrong bypass rule, resource change, or address update can make the control plane itself unreachable. The [write-authorization model](#the-write-risk-model) is therefore load-bearing, while milestone 1 remains read-only.

**Implementation status, updated 2026-08-16:** milestones 1–7 are implemented. The Pangolin adapter, intent/check-mode reconciler, four-tier write policy, ledgered additive writes, dynamic-IP aliases, reversible retirement, and provisioning-template engine all ship. Org/site/resource/rule/target reads have bounded live-read evidence. Write paths are comprehensively fixture-tested but have no independent live-write evidence; `loxep-acj.9` tracks the controlled first-write protocol.

:::note[M1 closeout — read this before M2]
Milestone 1 source-verified the endpoint and envelope model and confirmed the unauthenticated error envelope with a bounded live request. The standalone Integration API was not reachable from the verification environment at that time; no hostnames, routes, guesses, or private network paths are part of the tracked record. The canonical resource path is `/resource`, and list endpoints other than DNS records nest their arrays under a named key plus `pagination`.
:::

:::note[M2 closeout — read this before M3]
**Reachability update (2026-08-16):** bounded read-only reconnaissance later reached the Integration API and verified the org/site/resource/rule/target mappings. Write paths remain unexercised live. M2's planner and service tests use fake ports/adapters. `proxy_resources` still expresses origin through `hosting_target_id`; provider target intent remains a separate modeling concern rather than an inferred deployment-topology fact.
:::

:::note[M3 closeout — the write-authorization gate, read this before M4]
Milestone 3 shipped the gate, not a Pangolin write verb. Accepted design decisions established the four-tier `infrastructure.provider_write_policy`, `assertWritePolicy`, and policy-independent `wouldLockOut` preflight. The same early BLOCKED state applies to Cloudflare and Purelymail, so deployment credential scope is evaluated locally rather than documented here.
...
Milestone 4's write leg is exhaustively fixture-tested against fake providers and a scratch database, with no independent live-create evidence. The first-write protocol must use a disposable resource and explicit supervision; `loxep-acj.9` tracks that verification separately from the completed implementation milestone.
...
`source: 'pangolin_site'` ships but remains **without independent live-detector evidence** and degrades to “no detection this round” on failure. `probePangolinConnection` is registered separately from fleet providers because Pangolin is a control-plane provider.
:::

:::note[M7 closeout — verified against fakes only, same caveat as M4]
Milestone 7's retirement leg is exhaustively fixture-tested against fake providers and a scratch database, with the same missing live-write evidence as milestone 4. `loxep-acj.9` tracks a controlled disposable-resource exercise for both additive and retirement verbs.
...
Every response is wrapped in `{data, success, error, message, status}`. Provider source registers this schema, and a bounded unauthenticated request confirmed its error-envelope shape. An RPC-style envelope means HTTP 200 does not imply success, so the adapter branches on the envelope before HTTP status.
...
Pangolin is an identity proxy, so a write that removes access can also remove the route used to repair it. That provider-independent risk—not one installation's topology—is why the planner protects the dashboard resource and Loxep's own fronting resource.
...
1. **Write authorization is a stored, per-connection policy that defaults to off.** `infrastructure.provider_write_policy` is admin-controlled and audited. It applies to Pangolin, Cloudflare, and Purelymail alike; deployment credential scopes are private state and are evaluated at runtime, never recorded in this design.
...
detector observes a new address for alias 'home'
   ...
   -> the operator sees that multiple rules across affected resources reference
      an address that changed and applies with one click
```
...
The constraint is **not** an adapter gap. Purelymail exposes no token scoping: one account token carries reads and destructive writes. Safety therefore comes from Loxep's per-connection policy, ledger, and explicit apply flow rather than assumptions about a particular installation's credential.
...
The same mechanism covers Cloudflare. Runtime credential scope is deployment-private; under `read_only`, record publishing blocks with the required provider scope instead of attempting a write and surfacing a later auth failure.
...
M1  READ ONLY                              SHIPPED 2026-08-15, loxep-acj.1
    ...
    bounded live reconnaissance VERIFIED the envelope and read mappings;
    the standalone Integration API had initially been unreachable from the
    verification environment, but subsequent bounded reads reached it.
    No deployment route, hostname, or network topology is part of this record.
...
M4  TIER-1 PANGOLIN WRITES             SHIPPED 2026-08-16, loxep-acj.4
    ...
    MECHANICALLY POSSIBLE and fixture verified, but it has no independent
    live-write evidence. The controlled first-write protocol still requires a
    disposable resource and explicit operator supervision; see the closeout
    note for the required evidence.
...
not an loxep-acj.N milestone
    ...
    shape. LIVE: the read-only methods have bounded live-read evidence;
    no provider inventory or deployment topology is retained in this design.
...
M1 and M2 deliver useful visibility without enabling provider writes. That is the argument for this ordering: **the read half is useful before the write half is safe.**
...
1. **Re-verify every Pangolin API fact before relying on it**, using current provider source plus a bounded, explicitly opted-in live read where appropriate.
2. **Keep the accepted write-authorization model in force** before adding or changing any provider write.
3. **Confirm the self-lockout predicate against representative rule fixtures** before it gates production changes; never record a deployment's actual rule set in tracked docs.
4. **Never test a write against a needed resource.** The first live write must use a disposable resource created for the exercise, under explicit supervision. `loxep-acj.9` tracks this verification.
5. **Verify reachability from the deployed runtime itself.** Browser reachability does not prove container/process reachability, and private bridge or tunnel details belong in local deployment notes.
