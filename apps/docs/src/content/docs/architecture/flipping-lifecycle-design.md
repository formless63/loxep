...
**Status update, OWNER REVERSAL (2026-08-17).** The "quick entry"/"one screen" language throughout this document (this paragraph, [the section below fixing it as the design target](#quick-entry-is-the-dominant-path-and-it-must-be-one-screen), the route map, open question 1, and the milestone list) describes a one-screen dialog that no longer exists: the owner has since removed it directly, and `/finance/expenses/new` — the two-pane page [Expense Entry and Document Intelligence Design](../expense-entry-design/) built as quick entry's *composition* counterpart — is now the SINGLE expense-entry path. See [that design's own reversal note](../expense-entry-design/#the-route-and-what-happens-to-quick-entry) for the full account. The design reasoning below (why capture must be fast, why the dominant path cannot demand navigation) is left as written and still motivates the page's own defaults; only the one-screen-dialog form it originally specified is gone.
...
**Status update: the write gap above is closed.** `apps/web/package.json` now declares `@loxep/inventory` (added alongside the Invoice Ninja billing work), so `@/server/inventory-functions.ts`'s WRITES call the real service layer — item creation, acquisition creation, cost allocation, part-out, and the "I bought this" acquisition→item→link transaction all commit for real today, not just the reads. The paragraph above is left as written because it accurately describes the milestone as it first shipped; treat this note as the current-truth correction.
...
**Status update: both M5 gaps above are now closed.** `packages/app/src/inventory-ebay.ts` is the poll executor (`createEbayPurchasePollExecutor`), registered in `registry.ts` against the `ebay_purchases` target type exactly as this section's follow-up describes, and migration `0018_purchase_idempotency.sql` adds the `acquisitions_connection_external_ref_uq` partial unique index. See [2a's implementation-status subsection](#implementation-status-loxep-dgf5) for its own matching update.
...
**Status note, OWNER REVERSAL (2026-08-17):** the one-screen dialog this section specifies is REMOVED — see [Expense Entry and Document Intelligence Design's reversal note](../expense-entry-design/#the-route-and-what-happens-to-quick-entry). `/finance/expenses/new` is now the only expense-entry surface. The design target below is left as written; it no longer names a dialog.
...
**Status update: both follow-ups above have since landed.** `@loxep/app` now declares `@loxep/inventory`, `packages/app/src/inventory-ebay.ts` is the poll executor this section's follow-up describes (registered in `registry.ts` against `ebay_purchases`, with a per-connection enable/disable surface), and migration `0018_purchase_idempotency.sql` adds `acquisitions_connection_external_ref_uq`. Scheduled polling and the concurrent-sync race are both closed; the two numbered gaps above are historical, describing the milestone as it first shipped.
...
3. **No confirm action for `acquisition_cost`/`inventory_intake` dispositions.** The schema, the disposition vocabulary, and the review UI's disposition picker all support the full eight-member set from day one — an operator CAN mark a line "Cost of a lot" or "Stock (inventory)" — but confirming one into a real `acquisition_costs`/`inventory_items` row needs an acquisition-lot picker (create new vs. attach to an existing draft lot) this milestone does not build. `apps/web/src/features/documents/constants.ts`'s `CONFIRMABLE_DISPOSITIONS` and `documents-functions.ts`'s `CONFIRMABLE_AS_EXPENSE` name the gap in code, not just here. **Status update: BOTH halves of this gap are now closed.** [Expense Entry and Document Intelligence Design M6](../expense-entry-design/#milestones) (loxep-cd3.6) built the `acquisition_cost` -> `acquisition_costs` half (`confirmCandidatesAsAcquisition` plus the picker). [M7](../expense-entry-design/#milestones) (loxep-ytu) built the `inventory_intake` -> `inventory_items` half (`confirmCandidatesAsIntake`) — a candidate dispositioned `inventory_intake` becomes an ACTUAL physical stock row directly, not a cost row later turned into one; the two dispositions have routed to different functions since that pass.
...
2. **What does `expenses.acquisition_cost_id` mean?** It shipped as a real foreign key with no writer, and Phase 5's own no-copy rule forbids the reading it was presumably built for — a `capitalize = false` acquisition cost posts where it sits and is never copied into `expenses`. Three readings: (a) it is the supersession pointer, set on a **voided** expense that was re-recorded as a capitalized cost; (b) it is the copy the no-copy rule forbids; (c) it is dead weight and should be dropped. *Recommendation: (a), documented, and written by the void-and-promote path this design defines.* It gives the column a real writer, it makes the promotion auditable, and it contradicts nothing. (b) must be refused explicitly or the double-count returns. Dropping it is the fallback if the owner finds (a) a stretch of the column's name.
   ...
   **RESOLVED (loxep-ytu), per the recommendation, reading (a).** `@loxep/accounting`'s `ExpensesService.promoteToAcquisitionCost` is the writer: it voids the expense (`voidExpense`'s own state transition) and stamps `acquisition_cost_id` in the same statement, refusing on an already-`posted` or already-`void` row exactly as `voidExpense` does. Reading (b) is explicitly refused — nothing here copies a `capitalize = false` cost into `expenses`; the new `acquisition_costs` row is created FIRST (`@loxep/inventory`'s `AcquisitionsService.addCost`, `capitalize: true`, `cost_class: 'goods'`) and the expense that named it is voided, never the reverse. See [Expense Entry and Document Intelligence Design M7](../expense-entry-design/#milestones) for the shape.
...
1. **`expenses.acquisition_cost_id` exists and Phase 5's own rule forbids its apparent purpose.** *(**RESOLVED (loxep-ytu)**: Phase 5's milestone 4 gave the column its first READER, on reading (a) — the `acquisition_cost` source-fact reader links the expense that names a cost as a `journal_entry_source_links` row with role `evidence`, so a promotion is visible from the ledger side. `@loxep/accounting`'s `ExpensesService.promoteToAcquisitionCost` is now that reader's writer, exactly on reading (a): a **voided** expense re-recorded as a capitalized `acquisition_costs` row, never a copy of a non-capitalized cost (reading (b), which stays explicitly refused). See [Expense Entry and Document Intelligence Design M7](../expense-entry-design/#milestones) for the shape. The original text, recording how the contradiction stood before this pass, follows.)* [Phase 5](../financial-schema-design/#expenses-and-receipts) states that non-capitalized acquisition costs *"are not copied into `expenses`"* and posts them where they sit — and the same design's `expenses` sketch carries a foreign key to `acquisition_costs`, which migration 0006 made real. Nothing writes it. Either the column has the supersession meaning this design proposes (OQ2), or it is a leftover from the copy the design rejected. This is the clearest live contradiction between two shipped decisions in the documentation.
...
M4  Documents and intake review       migration. Depends on M2 (a confirm target must
                                      ...
                                      were NOT shipped here (needed an acquisition-lot
                                      picker) — closed by
                                      [Expense Entry and Document Intelligence Design
                                      M6](../expense-entry-design/#milestones)
                                      (loxep-cd3.6): confirmCandidatesAsAcquisition in
                                      @loxep/inventory plus the picker. The candidate ->
                                      inventory_items path (confirmCandidatesAsIntake, as
                                      distinct from the candidate -> acquisition_costs
                                      path M6 built) is ALSO now shipped — see [Expense
                                      Entry and Document Intelligence Design
                                      M7](../expense-entry-design/#milestones)
                                      (loxep-ytu).
