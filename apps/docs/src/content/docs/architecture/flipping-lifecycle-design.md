...
**Implementation status: DESIGN ONLY.** No migration, Drizzle schema, service, route, or component is authorized by this page. Every column type, constraint, and provider call named below must be re-verified against current PostgreSQL/Drizzle/provider behavior immediately before implementation, per the [dependency policy](../../development/dependency-policy/).
