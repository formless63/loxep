...
**Implementation status — milestone 1 is IMPLEMENTED and PROVISIONAL (loxep-lmy.1).** Milestones 2 and 3 remain design only. What shipped, what diverged, and what the owner still has to decide:
...
- **`@loxep/integration-cloudflare`** implements zones and DNS records: the five-kind taxonomy duplicated per ADR-0009, a per-connection rate budget, per-response redactors, and `read` / `apply` / `capabilities`. Token minting is milestone 3. Every endpoint, bound, and vocabulary was verified against developers.cloudflare.com and the official `cloudflare/api-schemas` OpenAPI document on 2026-08-13; seven facts that could not be confirmed are marked UNVERIFIED in the source. Two of this page's assumptions were **falsified** by that check and are corrected below.
- **`@loxep/infrastructure`** implements the materializer, the pure diff, the reconcile run, drift persistence, the intent services, and the idempotency ledger. **Its workspace manifest does not exist yet** — the source is complete and its 89 tests pass, but `packages/infrastructure/package.json` and the root typecheck/test aggregate entries are still owed, and until they land `@loxep/app` cannot import it. The composition-root wiring (executor + `target_type` routing) is therefore the one piece of milestone 1 that is written up but not shipped.
...
Everything below remains the specification. Exact column types, constraints, and provider endpoint behavior must be re-verified against current upstream sources immediately before implementing the remaining milestones, per the [dependency policy](../../development/dependency-policy/). Several [open questions](#open-questions) are marked **OWNER-REVIEW-CRITICAL** because they are unrecoverable once a zone, a token, or a mailbox exists at a live provider.
