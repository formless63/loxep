...
- Still open, unchanged: [OQ6](#open-questions) (is `provider_operations` promotable to shared foundation), OQ8 (registrar adapters), OQ9 (run-step retention), and OQ10 (live plan verification). `live-cloudflare.test.ts` is explicitly opt-in and skips cleanly without test credentials; tracked documentation does not record whether any deployment credential exists.
...
- The Purelymail contract and required DNS record set are source/fixture verified, with one bounded live probe. Complete read/write verification remains outstanding. Purelymail has no token scoping, so safe verification must be bounded by the invoked read-only call list rather than assumptions about a deployment credential.
...
- **What did NOT ship, and why it is a real gap rather than a silent one:** `@loxep/integration-cloudflare` has no token mint/roll/policy endpoints, so the corresponding `DnsTokenProviderPort` methods fail honestly with `provider_unavailable`. Zone and record reads have bounded live evidence. Later amendments below record the Pangolin service and task registration that closed the separately reserved reverse-proxy gap.
...
- **AMENDED 2026-08-16 — the reverse-proxy/tunnel gap is fully implemented, including its write leg.** Pangolin uses the shared four-tier write policy, policy-independent `wouldLockOut` preflight, ledgered additive writes, alias fan-out, provisioning templates, and reversible retirement. Read mappings have bounded live evidence; all write tiers remain fixture verified with the controlled first-write protocol tracked as `loxep-acj.9`. No private route, credential, or topology details are retained here.
...
- **AMENDED 2026-08-20 — attaching an existing provider zone closes the common `external_zone_id` gap (`loxep-8f8`).** `ManagedDomainsService.attachZone` records an explicitly selected zone, refuses silent replacement, and enqueues nothing because adoption changes no provider state. A bounded name-filtered read supplies candidates; create-zone and route-through-template remain separate future paths.
...
- **AMENDED 2026-08-16 — `hosting_targets` gains a typed multi-address model, and `address_v4`/`address_v6` are gone (`loxep-bub`).** The old DNS-publishable address pair could not represent private-network or provider-observed addresses honestly. Migration `0029_host_addresses` replaces it with typed `wan | lan | tailnet | other` rows, address family, provenance, and primary-per-kind semantics.
  ...
  **Observers fill their kinds automatically for a linked target and can never promote themselves to materializer-trusted provenance.** Tailscale writes `tailnet`/`observed:tailscale`; Dockhand writes parseable addresses as `other` with provider provenance. `classify()` may change kind but never provenance, keeping observed data structurally separate from operator-declared WAN intent.
