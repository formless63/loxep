...
- **AMENDED 2026-08-15 — the reverse-proxy/tunnel gap is now designed, not merely reserved.** [Pangolin Integration & Chain-Provisioning Templates](../pangolin-chain-design/) specifies `@loxep/integration-pangolin`, the `ProxyProviderPort` and its pure planner, the proxy intent tables, the service behind `infrastructure.sync-proxy-resource`, and the provisioning-template engine that sequences the whole registrar-to-running-resource chain by compiling into *this* document's reconciler, ledger, and run history rather than introducing a second execution engine. It is design-only; `sync-proxy-resource` stays unregistered until its milestone 2. Its write-authorization model is OWNER-REVIEW-CRITICAL for a reason no earlier provider raised: a wrong Pangolin write can remove the owner's own access to the estate, Loxep included.
- **AMENDED 2026-08-15 — the reverse-proxy/tunnel gap is now PARTIALLY closed (`loxep-acj.2`, the Pangolin chain design's milestone 2).** `hosting_targets.proxy_connection_id` (reserved above since migration `0012`) drives something real for the first time: migration `0027` adds `proxy_resources`/`proxy_resource_rules` (this page's own `dns_records.owner` per-row-ownership precedent, applied to a rule set), `reconcile_runs.subject_type` widens to include `proxy_resource`, `@loxep/infrastructure`'s `proxy-port.ts`/`proxy.ts` implement the port and its reconciler, and `infrastructure.sync-proxy-resource` is finally registered in `@loxep/app`'s `registry.ts`. It is CHECK MODE ONLY — the service refuses `mode: 'apply'` unconditionally, because the write-authorization gate this page's own `dns_provider_tokens`/`syncPolicy` split anticipated (milestone 3, `loxep-acj.3`) has not shipped, and the milestone-1 Pangolin adapter has no write verb to call regardless. No `proxy_drift_findings` table — proxy drift rides the reconciler's plan (`unmatchedObserved`), the same "no drift table, the plan already carries it" shape `ContainerHostPlan` established, rather than a second copy of `dns_drift_findings`' pattern.
- **AMENDED 2026-08-16 — the reverse-proxy/tunnel gap is now FULLY closed, including its write leg (`loxep-acj.3` through `.7`, all CLOSED).** The CHECK-MODE-ONLY restriction the previous amendment recorded is gone: M3 shipped the write-authorization gate this page's `dns_provider_tokens`/`syncPolicy` split anticipated — `infrastructure.provider_write_policy`, a registered setting with four ordinal tiers (`read_only` default), applied to Cloudflare and Purelymail too, not only Pangolin — plus `wouldLockOut`, a pure self-lockout preflight with no policy parameter, unconditionally refusing a write to Loxep's own fronting resource or Pangolin's own dashboard resource. M4 wired real tier-1 writes (`createResource`/`addTarget`/`createRule`) into `proxy.ts`'s apply leg, each ledgered through `provider_operations` exactly as this page's own token-mint pattern is. M5 added the alias primitive Pangolin itself lacks (`infrastructure.ip_aliases`, a registered setting) and owner-approved auto-apply for its ADD half only. M6 added `provisioning_templates`/`_steps`/`template_runs`/`template_run_steps` (migration `0028`) — a compiler-and-driver template engine that sequences THIS page's own reconciler, ledger, and run history rather than introducing a second execution engine, closing a `domain.declare` gap of its own (writing `managed_domains.provider_zone_status`, previously left to a `poll-delegation` task this page's own ordering deferred). M7 added the tier-2 RETIRE half (disable, never delete — Pangolin's rule `enabled` flag removes the need for any delete verb this page's own `provider_operations` design would otherwise have had to ledger). **No milestone through M7 has ever executed a write against the owner's real instance** — the Integration API's reverse-proxy route was unreachable from the build network M1's reconnaissance ran in, so every write-path test drives a fake adapter. That live-write leg is tracked separately as `loxep-acj.9`. Full detail, including the three owner rulings and each milestone's own implementation-status note, lives in [Pangolin Integration & Chain-Provisioning Templates](../pangolin-chain-design/) — this page is not restated further.
- **AMENDED 2026-08-18 — `materialize-records` and `sync-records` are REGISTERED, and DNS apply is reachable end to end for the first time (`loxep-vdt`).** Milestone 1's note above says the composition-root wiring "now ships too", and for the recurring drift sweep it did. It did **not** for the two tasks this page's own [job graph](#job-graph) puts at the centre of the intent path: `infrastructure.materialize-records` and `infrastructure.sync-records` were enqueued from `domains.ts`'s `create`/`updateIntent`, from `mail-sync.ts`'s ownership-code step, and from `/infrastructure/domains/$name`'s "Sync now" and a run's "Retry" — while **neither name had a handler in `registry.ts` at all**. Graphile Worker cannot resolve an unregistered identifier, so every one of those jobs burned its 25 attempts and died behind a success toast: creating a managed domain never materialized a single `dns_records` row. `@loxep/app`'s `infrastructure-domains.ts` now wires both — materialize reads intent, `host_addresses` (through `wanAddressPair()`), the CAA policy, and the mail provider's required set into `materializeDesiredRecords`, writes through `applyMaterializedRecords`, and chains the sync **in the same transaction**; sync resolves the domain's own `dns_connection_id` and runs `createRecordSyncService(...).run(...)`. The chained run is `mode: 'apply'`, `trigger: 'intent_change'` — the same shape `container-hosts.ts`'s `declareIntent` already uses, and gated by the same `infrastructure.provider_write_policy` tier, so on a fresh installation it is refused as a `'blocked'` step and the run finishes `'partial'` until an admin raises that connection's tier. The recurring sweep is unchanged and stays `'check'`. **One case is still not reachable**: `run()` refuses a domain with no `external_zone_id`, which today only the milestone-6 template engine ever sets, so a domain created from `/infrastructure/domains/new` materializes its records and stops there — the handler logs and returns rather than retrying, and `requestDomainResync` refuses up front instead of queueing a job that cannot start. `@loxep/storage`'s `storage.migrate-object` was registered in the same change (it had been built with `defineTask` and never composed), and `packages/app/test/registry-completeness.test.ts` now asserts the general property — every `*_TASK`/`*_TASK_NAME` constant declared under `packages/-/src` has a registered handler — so this class of bug cannot recur.
- **AMENDED 2026-08-20 — the `external_zone_id`-null dead end `loxep-vdt` made honest is now CLOSED for the common case (`loxep-8f8`).** The gap above named three ways to close it: (a) build `infrastructure.ensure-zone` as a real task that adopts-or-creates the Cloudflare zone, (b) an operator "attach an existing zone" action for a zone that already exists at the provider, (c) route the plain new-domain form through a provisioning template. **This ships (b)** — the owner's zones already exist at Cloudflare, `@loxep/integration-cloudflare` already exports everything a read needs (`listZones`, `getZone`), and it is the cheapest of the three. `ManagedDomainsService.attachZone` (`packages/infrastructure/src/domains.ts`) writes `external_zone_id` (+ `provider_zone_status`/`zone_nameservers` when the caller has them) onto a domain, admin-only, idempotent for the SAME zone id, and refusing to silently overwrite an already-attached DIFFERENT zone unless the caller passes `replace: true` — an operator re-pointing a domain at a different zone is a real, deliberate act. It enqueues **nothing**, per Rule P11 (Estate Browsers Design §2.5, adopt-into-intent): attaching changes nothing at the provider and does not itself start a reconcile — the operator's existing "Sync now" (`requestDomainResync`) is the separate next step, and it is what stops refusing once a zone is attached. `apps/web`'s `fetchCandidateManagedDomainZones`/`attachManagedDomainZone` (`infrastructure-functions.ts`) mount this: a bounded, name-filtered `listZones` read (one page, the same narrowing `dispatchDomainDeclare`'s `findZoneByName` already relies on) plus the write, surfaced on `/infrastructure/domains/$name` as "Attach zone…" (quietly "Change zone…" once attached) and a blocked-state banner reusing `requestDomainResync`'s own refusal wording. **(a) build-the-zone and (c) route-through-template remain open** — this bead does not build `ensure-zone`/`provision-domain` as tasks, and a domain declared with no matching Cloudflare zone still has no self-service way to CREATE one from `/infrastructure/domains/new` (only the milestone-6 template engine's `domain.declare` step does that, by resolving-and-blocking rather than creating).

- **AMENDED 2026-08-20 — `materialize-records` now owns its `reconcile_runs` row, closing the observability gap `loxep-vdt` deliberately left (`loxep-ejs`).** The `loxep-vdt` entry above says plainly that a `MaterializationError` "currently surfaces only as a failed job in the worker log" because building a run row from `@loxep/app` would mean writing `reconcile_run_steps` from the composition root — `@loxep/infrastructure`'s job, not the app's. `packages/infrastructure/src/materialize-run.ts` (`runMaterializeRecords`) is the fix: it now owns the WHOLE verb — the `MaterializeInput` assembly (`hosting_targets`/`host_addresses` via `wanAddressPair()`, the CAA policy, the mail provider's `requiredRecords()`, moved out of `@loxep/app`'s `infrastructure-domains.ts` and into this module since none of it needs an integration package, only the two structurally-declared provider ports `DnsProviderPort`/`MailProviderPort` already used elsewhere in this package), the pure `materializeDesiredRecords` call, `applyMaterializedRecords`, and the chained `sync-records` enqueue (still one transaction, unchanged) — wrapped in `reconcile_runs`/`reconcile_run_steps` writes the same way `sync.ts`/`mail-sync.ts`/`container-hosts.ts` already wrap their own verbs. `@loxep/app`'s `infrastructure-domains.ts` is now a thin wrapper matching `infrastructure-token.ts`'s and `infrastructure-mail.ts`'s own shape: resolve the real Cloudflare (and, lazily, Purelymail) adapters this domain needs, bridge them to the structural ports, call `runMaterializeRecords`. No migration: `reconcile_runs.kind` is an open set (no `CHECK`), and `subject_type = 'domain'`/`mode = 'apply'` were already in the closed sets those two columns validate against. The run's `kind` is `'materialize-records'`, always `mode: 'apply'`/`trigger: 'intent_change'` (the only trigger any real caller enqueues), with steps `read-intent` → `materialize` → `apply-records` → `enqueue-sync`; a `MaterializationError` (a fronting cycle, a tunnel client with no fronting node, a hosting target with no publishable address, a private Tailscale-range address in a `wan`/`operator_declared` row, or a proxying intent the provider cannot honor) is thrown by the pure decision BEFORE any `dns_records` write, so the run finishes `'failed'` with the `materialize` step carrying `errorCode: 'materialization_error'` and the exact reason, and `dns_records` is left completely untouched rather than half-published — now visible on `/infrastructure/runs` and a run's detail page exactly like every other reconciler's failure.

- **AMENDED 2026-08-16 — `hosting_targets` gains a typed multi-address model, and `address_v4`/`address_v6` are GONE (`loxep-bub`).** The owner's own framing: *"most of dockhand is showing tailscale ip, which might not be the most helpful in all scenarios"* — the single address pair this page's own [`hosting_targets` sketch](#hosting_targets) shipped meant exactly one thing, *the* DNS-publishable WAN address, and had no column for a LAN IP, a Tailscale IP, or anything a companion sync merely observed. Migration `0029_host_addresses` adds `host_addresses` (`hosting_target_id`, `kind` ∈ `wan | lan | tailnet | other`, `family` ∈ `v4 | v6`, `value inet`, `provenance` = `'operator_declared'` or `'observed:<provider>[.<field>]'`, `is_primary` scoped per `(kind, family)`) and, pre-release, DROPS `address_v4`/`address_v6` after backfilling every existing value into a `wan`/`operator_declared`/primary row — the one alteration to an EXISTING table this document's "no existing table gains a column" rule has ever recorded, because the honest description is a replacement, not an addition.

  **The CGNAT/tailnet guard (this page's [materializer rule](#constraints-the-materializer-must-enforce) above) becomes STRUCTURAL, not just enforced.** `@loxep/infrastructure`'s `wanAddressPair()` (`host-addresses.ts`) is the one function that turns a target's `host_addresses` rows into the materializer's `addressV4`/`addressV6` pair, and it reads exactly `kind = 'wan' AND provenance = 'operator_declared'` — nothing else. A `tailnet` row, however it got there, cannot influence a DNS record no matter what is stored, because the filter that builds the materializer's input never reads it; `resolveHostingAddress`'s CGNAT/ULA publish-guard (`tailnet-address.ts`) stays as defense in depth for the one case the filter cannot catch — a private-range VALUE hand-typed into a WAN-declared row. `hosting_targets_addressable_check` (the CHECK this page's sketch gave the old columns) cannot survive as a CHECK — PostgreSQL cannot query a child table — and is re-expressed as a service-level invariant in `targets.ts` (`create()`) and `host-addresses.ts` (`remove()`/`classify()`), narrowed to the same WAN-only semantic the old columns always carried.

  **Observers fill their kinds automatically for a LINKED target, and can never promote themselves to what the materializer trusts.** `projectTailscaleDevices` (`@loxep/app`'s `fleet-health.ts`) now writes/refreshes `kind = 'tailnet'`, `provenance = 'observed:tailscale'` rows for a device's linked hosting target — the tailnet SLOT is quarantined by kind+provenance together, so the "suggest, not overwrite" tension `loxep-rf4`'s Tailscale weave design §3.2 and this page's own address-suggestion prohibition raised earlier simply does not apply here: writing a `tailnet` row is not writing `address_v4`/`address_v6`, and never can be. `projectDockhandResources` lands its `host`/`publicIp` fields as `kind = 'other'`, provenance `observed:dockhand`/`observed:dockhand.public_ip` — heuristically unclassified by design, exactly per the owner's note about Dockhand's tailscale-IP habit — for any value that parses as a real IP literal (`host` is a connection address and is often a hostname, which the `inet` column cannot store; a non-IP value is silently skipped rather than written malformed). `classify()` may change a row's `kind`; it can NEVER change `provenance` away from `observed:*`, which is the entire quarantine in one rule.

  `/infrastructure/fleet/$name` gains a typed address card (kind badges, provenance, the observer's clock, and declare/classify/set-primary/remove, admin-gated) below the existing header card; the create dialog's `addressV4`/`addressV6` fields are unchanged in the UI but now write `wan`/`operator_declared`/primary `host_addresses` rows through `targets.ts`'s `create()`, in the same transaction as the target row. `@loxep/infrastructure`'s test suite gains `host-addresses.test.ts`, including the structural-quarantine proof this bead's own scope names: a `tailnet` row present never reaches a materialized record plan.
...
**Registration status (2026-08-18, `loxep-vdt`).** Seven of these nine names have a registered handler in `@loxep/app`'s `registry.ts`: `materialize-records`, `sync-records`, `sync-token-policy`, `ensure-mail-domain`, `poll-mail-ownership`, `sync-mailboxes`, and `sync-proxy-resource`. The first two were registered late — they had been *enqueued* since milestone 1 with no handler at all, which is silent by construction, and the amendment in this page's implementation-status header at the top of this page records what that cost. `provision-domain`, `ensure-zone`, and `poll-delegation` remain unbuilt as tasks; milestone 6's provisioning-template engine performs the equivalent work as compiled steps of a `template_run` instead.

**`materialize-records` records a run too (2026-08-20, `loxep-ejs`).** Registration alone did not give `materialize-records` a `reconcile_runs` row — `sync-records`, `sync-token-policy`, `ensure-mail-domain`/`poll-mail-ownership`/`sync-mailboxes`, and `sync-proxy-resource` all recorded their own runs from day one, and `materialize-records` was the one exception, so a `MaterializationError` was invisible outside the worker log. See this page's implementation-status header for the fix: `@loxep/infrastructure`'s `runMaterializeRecords` now owns that write, the same way every other name in this list already does.

**`external_zone_id` update (2026-08-20, `loxep-8f8`).** The column `sync-records` requires before a reconcile run can start is no longer written by the template engine alone. `ManagedDomainsService.attachZone` is a second writer — a request-scoped ADMIN ACTION, not a task, mirroring the same "mint/roll are request-scoped, never jobs" split `tokens.ts` established for token minting. It lets an operator attach a zone that already exists at the provider, closing the `/infrastructure/domains/new` dead end for the common case without building `ensure-zone`/`provision-domain`. See this page's implementation-status header for the full account.

A name in this table with no registered handler is a **bug, not a placeholder**: `packages/app/test/registry-completeness.test.ts` fails the build if a `*_TASK`/`*_TASK_NAME` constant exists anywhere under `packages/-/src` without a handler. Reserving a contract ahead of its implementation — the way `sync-proxy-resource` was reserved for two milestones — means reserving the payload *shape*, not exporting a name that product code can enqueue into a void.
