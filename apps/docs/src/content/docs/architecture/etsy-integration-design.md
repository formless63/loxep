...
**Status: design only. No code in this change.** The eBay integration (`packages/integrations/ebay`) is the load-bearing reference throughout — its error taxonomy, rate budget, keyset/consent split, and order-fact mapping are the pattern this document adapts, not reinvents.
