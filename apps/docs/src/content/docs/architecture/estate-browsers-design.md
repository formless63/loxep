...
This document designs the **estate browser** program (`loxep-47o`): a consistent, low-friction way to inspect provider state and use existing Loxep-owned actions without switching among provider dashboards.
...
**Status: DESIGN, with Wave 0 and Wave 1a now SHIPPED on top of it.** [Section 9](#9-decomposition-into-child-beads) names the beads that ship it. High-risk affordances remain subject to provider scope, write policy, admin gating, and typed confirmation; [section 8](#8-owner-review-critical) collects those decisions.

**Implementation status, updated 2026-08-16 (`loxep-47o.1`/`loxep-47o.2` shipped).** The shared `/<workspace>/estate/$connectionId` shell, provider registries, section/error primitives, index, and **Open estate** action are implemented. Cloudflare zones and record drill-ins use the shared DataTable stack, cross-reference Loxep intent, and adopt records through the existing `ManagedDomainsService.addManualRecord` path. The adapter re-walks earlier pages when its cumulative `maxPages` ceiling increases; bounded live-read evidence confirms zone and record mapping without retaining account inventory.

**`loxep-47o.4`/`loxep-47o.5` shipped, 2026-08-16.** Dockhand environments are cross-referenced against `external_resources`/`resource_links`; container and stack drill-ins key directly on the provider environment and reuse the existing adopt-as-hosting-target action. Gatus renders its open/basic/OIDC posture, treats structural OIDC refusal as BLOCKED, excludes the outward-heartbeat keys through the shared derivation, and supports per-endpoint uptime drill-ins. Bounded live reads validate the provider mappings; link-state and heartbeat-exclusion branches are covered deterministically rather than documented from one installation's state.

**`loxep-47o.6`/`loxep-47o.7` shipped (2026-08-16).** Tailscale renders linked, ignored, and candidate devices keyed by node id and reuses the existing Link/Declare/Ignore actions. Beszel renders hub and system reads with the existing attach action. Termix renders hosts and the accepted instance-wide sessions view without write actions. The three providers share one tested cross-reference helper and independently budgeted adapter accessors. Bounded live reads verify each adapter-to-estate DTO path without recording connection identities, inventory, or topology.

**`loxep-47o.8` shipped (2026-08-16).** The Invoice Ninja estate ships at `/finance/estate/$connectionId` — the FIRST estate page outside `/infrastructure`, which is what actually proves Rule P1's workspace parameter rather than merely documenting it: the route, the shared shell (`EstateHeader`/`EstateSection`/`EstatePage`), and `@/features/estate/provider-registry`'s metadata map are all exactly the ones every infrastructure estate page already uses, and `features/finance/estate/section-registry.tsx` is the first per-workspace section registry built outside `features/infrastructure/estate/`. Clients (`fetchClientsPage`) and Invoices (`fetchInvoicesPage`) are the fixed two-call overview, each cross-referenced against the REAL `resource_links` rows the shipped `pushDraftInvoice` flow already writes — `purpose='billing_client'` for a client's counterparty projection, `purpose='billing_invoice_draft'` for an invoice's draft-push target (`finance-billing.ts`'s own vocabulary, reused rather than re-derived). Invoice Ninja's list endpoints paginate by real page number (`meta.pagination`), so each server function fetches exactly one page per call; "Load more" (Rule P8) adds exactly one page number to the client's per-page query set and costs exactly one call, never a re-walk of pages already loaded. Per-row drill-ins (`fetchClient`/`fetchInvoice`) add exactly what the overview row omits — `contacts[]` and `lineItems[]` respectively — never a duplicate read. Zero write affordances: none of `createClient`/`updateClient`/`createInvoice`/`updateInvoice`/`markInvoiceSent` (a GET that mutates) is imported anywhere in the new code, and a linked invoice row's "Loxep record" cell links to `/finance/overview` — the existing push dialog's home — rather than growing a second entry point. One finding worth recording: this app has no counterparty detail ROUTE at all today, so a linked client/invoice's counterparty renders as plain text (name plus reference code) rather than a link — Rule P11's "link to the Loxep record where one exists" degrades honestly to "name the Loxep record" when no such route exists to link to.
...
**Rule P3b — the cross-reference speaks observationally (AMENDED 2026-08-17, owner ruling, `loxep-2mr`).** Loxep is a layer that streamlines interaction with connected services, not an exclusive control surface, and the cross-reference column's vocabulary must not imply otherwise. The column is headed **"In Loxep"** everywhere (retiring the judgey variants — "Managed by Loxep", bare "Loxep") and its states are neutral observations: *linked*, *declared*, *drift finding open*, *observed only*, or a quiet em-dash — never "Not declared"/"Not linked" phrased as a deficiency. Linking or declaring is an **optional upgrade** — the moment an operator wants Loxep to start applying settings outward — and its affordances are framed that way ("Link…", "Declare…", adopt verbs unchanged in function), never as the price of visibility. What stays LOUD is the outbound-automation surface: write-policy tiers, auto-apply flags, enqueued reconciles, and drift against records Loxep itself declared (a reconciler may overwrite those — the operator must see it coming). Drift that is merely *"exists at the provider, Loxep never declared it"* is information, not an alarm: it renders in a neutral/warning tone, never destructive — the `proxy-resource-row` already states the model sentence: *"information, not drift to correct."*

**Implementation status (`loxep-2mr`, 2026-08-17).** Shipped: the "In Loxep" header unification (retiring "Managed by Loxep" and bare "Loxep") across `cloudflare-estate/{zone,record}-columns.tsx`, `gatus-estate/endpoint-columns.tsx`, `termix-estate/hosts-columns.tsx`, `dockhand-estate/environment-columns.tsx`, and `purelymail-estate/{domains,mailboxes,routing-rules}-columns.tsx`; "Observed only" replacing every "Not declared"/"Not linked" cell state; `DRIFT_KIND_TONE.unexpected` retoned `secondary` (was `destructive`) in `features/infrastructure/constants.ts`, mirroring `record-columns.tsx`'s own cross-reference tone for its identical `unexpected` state (that file's "Declared / Drift finding open / Unexpected" states were already this rule's reference implementation — only its header was retitled); `unmatched-container-hosts-card.tsx`'s title reframed to "Observed Dockhand hosts"; the unmatched-tailnet-devices panel's empty state reframed away from "needs review" ("Nothing left to link right now"), ignore mechanics unchanged. `dns-drift-panel.tsx` and `proxy-resource-row.tsx` needed no edit — already calm and factual per this rule.
...
**Writes today.** `mail-sync.ts`'s two domain-level verbs are gated at tier 1 with reason `credential_scope` and mounted on any domain row already registered on THIS connection: `runMailDomainSync` ("Sync now") and `runMailboxSync` ("Sync mailboxes"), both rendering policy-blocked at the connection's current `read_only` tier. `loxep-47o.11` (shipped 2026-08-17) built the per-row service verbs the original wave left as a follow-up bead (`packages/infrastructure/src/mailbox-admin.ts`'s `MailboxAdminService`) and mounted two of them, per the generalized write-policy ruling ([8.1](#81-purelymail-writes)):

- **Mailboxes section, "Delete…"** — `deleteMailboxNow`, destructive, gated at `access_affecting`-or-higher EXPLICITLY (never the `additive` tier `runMailboxSync`'s own batch delete uses), typed confirmation of the full address, re-checked server-side. Mounted only on a row Loxep has a `mailboxes` intent row of kind `mailbox` for (the reconcile run needs a `managed_domains.id` subject, same gate `PurelymailDomainRowActions` already established).
- **Routing rules section, "Delete…"** — `deleteRoutingRule`, same tier and confirmation shape, checked against the rule's own `<matchUser>@<domainName>` pattern read FRESH from the provider (never the client's copy). Same Loxep-row gate.
- **`modifyMailbox`** — **NOT built at all.** The bead that filed this work assumed `PurelymailAdapter` already exposed a `modifyUser` method alongside `createUser`/`deleteUser`/`createRoutingRule`/`deleteRoutingRule`. It does not — `packages/integrations/purelymail/src/adapter.ts`'s `PurelymailAdapter` interface has no `modifyUser` member; `"user.modify": "modifyUser"` exists only in `operations.ts`'s raw name map, in the same deliberately-unwired category as `appPassword.create` and the password-reset operations. Building it would require adding a method to the adapter, which sat outside `loxep-47o.11`'s fence (the adapter package was read-only to that work). Recorded here rather than silently dropped, per this doc's own discipline — see [8.1](#81-purelymail-writes).

**`loxep-4xo`, shipped 2026-08-18 — the create half of the asymmetry.** `loxep-47o.11` shipped delete-only, naming `createRoutingRule` as built-but-unmounted "pending a design decision" and leaving `MailDomainsService.addMailbox` unwired entirely — an operator could DELETE a mailbox or routing rule from this page but never CREATE one. This wave makes the design decision the previous wave deferred: both sections gained a SECTION-level create affordance (there is no existing row to act on when creating one, unlike the row-level deletes above), rendered via `EstateSection`'s new optional `headerAction` slot (additive prop, every other estate section that omits it is unchanged):

- **Mailboxes section, "New mailbox…"** — mounts `MailDomainsService.addMailbox` (`packages/infrastructure/src/mail.ts`), NOT `MailboxAdminService` (there is no `createMailboxNow` — Purelymail's own API has no single-mailbox-create outside the reconciler's whole-domain convergence). A Loxep-OWN intent write: upserts the `mailboxes` row and enqueues `infrastructure.sync-mailboxes`, the same task `enableMailForDomain`/`applyDefaultMailboxTemplate` already enqueue — the actual provider `createUser` call happens later, inside `runMailboxSync`, which is ALREADY gated. No write-policy tier check at this layer (matches `enableMailForDomain`'s own precedent), so this affordance is never rendered policy-blocked — only disabled when the connection has no domain Loxep declares yet (a real precondition, not a policy flip).
- **Routing rules section, "New routing rule…"** — mounts `MailboxAdminService.createRoutingRule`, additive (tier 1), the SAME write-authorization gate the row `deleteRoutingRule` uses one tier lower, no typed confirmation (additive, never destructive). Renders Rule P14's visibly-blocked state from the same `estateConnectionSummaryQuery` tier the row deletes already read, before any click.

Both mounted verbs render Rule P14's visibly-blocked state from the header's own write-policy tier, before any click — no surprise post-click refusal.

**Permanently read-only here.** `deleteDomain` (present in `operations.ts`, deliberately never called by Loxep — `loxep-47o.11`'s own boundary test asserts `mailbox-admin.ts` never calls it), `appPassword.create` and the password-reset operations (deliberately unimplemented; `appPassword.create` is the one Purelymail response documented to return a credential, and [ADR-0022](../../decisions/0022-minted-secret-reveal/)'s reveal-once channel does not reach the reconciler's mint), and `modifyUser` (does not exist on the adapter at all — see above). Loxep never sends or reads mail through this connection.

**Mailbox templates (`loxep-4xo`).** `applyDefaultMailboxTemplate` (the domain detail page's Mail panel) has applied a `mailbox_templates` row since Phase 7 milestone 2, but `createMailboxTemplatesService` was never constructed in `admin.ts` until this bead — nobody could see, list, or author what it applies. Now wired (`getMailboxTemplatesService`) and rendered READ-ONLY on the Mail panel's "View templates" dialog: name, default flag, and each entry's local part/kind/forward-to — exactly what applying a template would create. Authoring is NOT built: `MailboxTemplatesService.create` takes a whole template in one call, but a legible multi-entry editor (add/remove rows, per-entry kind/forwardTo validation, `isDefault` exclusivity) is a real form on its own, left as a follow-up.
...
**Sections.** Sites (status, newt liveness), resources (fullDomain, mode, enabled, SSO/auth posture, rule counts, cross-referenced against declared `proxy_resources`), org domains. **Exactly three calls regardless of estate size**; the three-call mapping has bounded live-read evidence.
...
**Open constraint.** No Pangolin write tier has independent live-write evidence (`loxep-acj.9`). Apply and retire therefore remain policy-gated and fixture verified.
...
**Shipped (`loxep-47o.4`, 2026-08-16, read-only).** Environments cross-reference `external_resources`/`resource_links` for a `'linked'` / `'unmatched'` / `'unknown'` verdict, with bounded live-read evidence. The containers/stacks drill-in reads by `externalHostId` directly, so it also works before an environment is attached to a hosting target. No lifecycle field exists on the DTO, and `test/forbidden-verbs.test.ts` remains the binding proof of the adapter surface.
...
**Shipped (`loxep-47o.5`, 2026-08-16).** The three-way posture is a display-only inference layered on the adapter's OIDC signal. Structural OIDC refusal renders BLOCKED while genuine credential rejection remains ERROR. Heartbeat exclusion reuses `gatusPushQuarantinedKeys()` rather than re-deriving it; live-read evidence covers the direct posture, while exclusion variants have deterministic coverage.
...
**Shipped (`loxep-47o.8`, 2026-08-16).** Exactly as specified above — see the implementation-status note near the top of this document for the full accounting (the real `billing_client`/`billing_invoice_draft` cross-reference, the one-page-per-call "Load more", the `contacts[]`/`lineItems[]` drill-in split, and the no-counterparty-detail-route finding).
...
**Rule N2 — one nav entry per workspace, never one per provider.** `config/navigation/infrastructure.ts` gains a single **Estates** item pointing at `/infrastructure/estate`, an index listing every infrastructure-category connection with its provider, health status, write-policy tier, and a link to its page. `/finance` gains the equivalent (`config/navigation/finance.ts`'s own **Estates** item, `/finance/estate`) — shipped with `loxep-47o.8`. There is deliberately **no** global cross-workspace estates index: `/settings/connections` already lists every connection of every provider, and a second global list would be a fifteenth surface duplicating it.
...
### Wave 3 — billing — SHIPPED

| Rank | Provider | Notes |
| --- | --- | --- |
| 8 | **Invoice Ninja** — SHIPPED (`loxep-47o.8`) | `/finance/estate/$connectionId`, read-only. First estate page outside `/infrastructure`, which is what proves P1's workspace parameter. |
...
- **Estate pages for link-only tools.** The accepted product rule is that tools without a real integration are not advertised. Netdata, Cockpit, and Uptime Kuma were deleted from `FLEET_TOOL_REGISTRY` for this reason; do not re-add them here.
...
**RESOLVED per the generalized write-policy ruling.** Purelymail API tokens are account-wide and **have no token scoping** — one token carries every operation, including `deleteDomain`. Deployments should therefore begin at `read_only`, use disposable targets for controlled write verification, and never treat the credential itself as a least-privilege boundary. `createUser` is also **billable and not idempotent** (no upsert exists), while `deleteUser` deletes the mailbox and its mail.

**Accepted design (2026-08-16, #3):** the per-connection write-policy tiers, admin gating, and typed confirmation are the authorization model; estate pages mount service-layer paths only (P10), never raw API passthrough. This supersedes the original recommendation above (ship read-only with no write question asked past "whether any write may originate here"): the Purelymail estate page mounts only existing, gated `mail-sync.ts` service verbs — `runMailDomainSync` ("Register domain") and `runMailboxSync` ("Sync mailboxes"), both tier 1 with `blockedReason: 'credential_scope'` because the provider token is unscopable. Every connection defaults to `read_only`, so both affordances render **policy-blocked** until an administrator explicitly raises that connection's tier. Mailbox/routing-rule row-level CREATE, UPDATE, or DELETE initiated from a single estate-page row had no existing service-layer path at all (Purelymail's own API has no single-mailbox-create outside the reconciler's whole-domain convergence) — filed as a follow-up child bead (`loxep-47o.11`) rather than built this wave.

**`loxep-47o.11` follow-up, shipped 2026-08-17.** Built `packages/infrastructure/src/mailbox-admin.ts`'s `MailboxAdminService` — `deleteMailboxNow`/`createRoutingRule`/`deleteRoutingRule`, ledgered into `reconcile_runs`/`reconcile_run_steps` the way `mail-sync.ts` is, `assertWritePolicy`-gated. The two destructive verbs (`deleteMailboxNow`, `deleteRoutingRule`) assert `access_affecting` (tier 2) EXPLICITLY — never the `additive` tier `runMailboxSync`'s own batch delete uses, and never `assertWritePolicy`'s bare default — plus a typed confirmation the SERVICE itself re-checks (package-testable, not only at the `apps/web` server-function layer, per the bead's own instruction). Mounted on the estate page: mailbox "Delete…" and routing-rule "Delete…", both Rule P14 visibly-blocked before any click. `createRoutingRule` (additive, tier 1) is built and tested but **NOT mounted** — this design names no sanctioned section-level create home on this page, so it stays service-layer-only pending a design decision. `modifyMailbox` was **NOT built at all**: the bead assumed `PurelymailAdapter` already exposed `modifyUser`; it does not (only `PURELYMAIL_OPERATIONS`'s raw name map names it, unwired, same category as `appPassword.create`), and adding it would have required editing `packages/integrations/purelymail`, which sat outside this work's fence. `deleteUser` therefore moves OFF the "permanently unreachable" list below (it is now reachable, behind tier + confirmation, via `deleteMailboxNow`); `deleteDomain`/`appPassword.create`/the password-reset operations/`modifyUser` remain on it. `createUser` still has no per-row mount (only the whole-domain convergence loop creates a mailbox) — not requested by this bead, not built.

**`loxep-4xo` follow-up, shipped 2026-08-18 — the pending design decision resolved.** `createRoutingRule` is now MOUNTED: a section-level "New routing rule…" on the Routing rules section (there is no existing row for a create to act on, unlike the deletes above), same additive/tier-1 gate, no typed confirmation. `MailDomainsService.addMailbox` (a separate, Loxep-own intent write — not `MailboxAdminService`, which still has no `createMailboxNow`) is mounted as "New mailbox…" on the Mailboxes section, with no write-policy tier check (the provider call happens later, inside the already-gated `runMailboxSync`). Both close the audit-flagged asymmetry: the estate page could DELETE a mailbox or routing rule but not CREATE one. `modifyMailbox`/`deleteDomain`/`appPassword.create`/the password-reset operations remain unbuilt/permanently unreachable, unchanged from the paragraph above.

### 8.2 Cloudflare record editing

Cloudflare write scope is a deployment fact and must not be documented here. An *edit this record* affordance would be a new raw write path, which P10 forbids independently of credential scope.
...
`pushDraftInvoice` guards only on a `resource_links` idempotency check; `createClient`/`createInvoice` reach the provider with no policy gate, and `invoiceninja` is absent from `WRITE_POLICY_ENFORCED_PROVIDERS`. Every write path is source/fixture verified but has no independent live-write evidence.
...
`loxep-acj.9` remains open because Pangolin writes have no independent live-write evidence. An estate page that renders Apply and Retire beside provider resources makes an accidental first write materially easier, so policy gating remains mandatory.
...
`TermixSessionFact.sharedByUsername` names a person. Per-host session rows were accepted for trusted deployments, but an **instance-wide** list of who is logged into what is a materially broader surveillance surface; `loxep-wvm` §3.3(a) already flagged the general shape.

**RESOLVED — accepted design 2026-08-16, item 5b: instance-wide Termix sessions are permitted for the same trusted-deployment posture as per-host sessions.** This supersedes the original recommendation below (ship hosts-only, wait for a grant) and `loxep-47o.7`'s original acceptance-criteria text. The instance-wide Sessions section shipped in the same change as the Hosts section (`loxep-47o.7`, 2026-08-16) — see §3.8's own "Shipped" note above — reusing the per-host panel's exact column set and DTO shape (`termixSessionColumns`, `TermixSessionRowDto`) rather than declaring a parallel render. `sharedByUsername` renders verbatim exactly as the per-host panel already does.
...
8. **`loxep-47o.8` — Wave 3: Invoice Ninja estate. SHIPPED.** [3.9](#39-invoice-ninja--financeestateconnectionid). `/finance/estate/$connectionId` — the first estate page outside `/infrastructure`, which is what proves P1's workspace parameter. Read-only. Depends on `.1`.
