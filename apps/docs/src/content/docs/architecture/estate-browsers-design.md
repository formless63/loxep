...
- **`createRoutingRule`** — built at the service layer (additive, tier 1) but **NOT mounted**: this design names no sanctioned section-level "create" home on this page (Rule P10 forbids inventing one), so it ships unmounted pending a design decision.
...
**Permanently read-only here.** `deleteDomain` (present in `operations.ts`, deliberately never called by Loxep — `loxep-47o.11`'s own boundary test asserts `mailbox-admin.ts` never calls it), `appPassword.create` and the password-reset operations (deliberately unimplemented; `appPassword.create` is the one Purelymail response documented to return a credential, and [ADR-0022](../../decisions/0022-minted-secret-reveal/)'s reveal-once channel does not reach the reconciler's mint), and `modifyUser` (does not exist on the adapter at all — see above). `createUser` has no per-row mount either (only the whole-domain convergence loop creates mailboxes); a per-row mailbox-create verb was not requested by `loxep-47o.11` and was not built. Loxep never sends or reads mail through this connection.
