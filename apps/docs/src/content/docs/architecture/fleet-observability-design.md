...
```text
connections.provider = 'gatus' | 'beszel' | 'dockhand'
...
- The base URL is **non-secret connection config**, exactly as a WooCommerce store URL is, and for the same reason: it must stay readable without a decryption round-trip. For Dockhand it is stored as the **origin**, with the unversioned `/api` prefix owned by the adapter — a pasted URL ending in `/api` is normalized rather than doubled.
- **Every one of these three is a login, and not one of them is an API token**, because not one of these tools issues one. The form labels must say so. The original warning here was about labelling a superuser password an "API token"; the correction to the Beszel verdict does not retire that warning, it generalizes it — Beszel's field is a readonly user, Dockhand's is a session login, and a form implying otherwise is the same failure.
- **Dockhand's account should hold four permissions and no more:** `environments:view`, `environments:edit`, `containers:view`, `stacks:view`. It must not be the operator's own admin account. Loxep's restraint lives in its own adapter surface, not in Dockhand's session — a session that can list containers can also start them, which is precisely why the [forbidden-verbs test](#dockhand-in-detail--amended-2026-08-13) exists.
- **Dockhand's login is the rate-limited endpoint**, not its reads: upstream locks out after five failed attempts per IP/username pair. A stored credential that has gone stale must therefore fail loudly and stop, never retry in a loop.
