...
**Implementation status — milestone 1 is IMPLEMENTED (loxep-ovj.1).** Milestones 2, 3, and 7 remain design only. What shipped, and how each "Before implementing this phase" item and open question was resolved for this milestone:
...
- **Not built in this milestone, on purpose:** the Gatus outward push (milestone 9's recommendation, still open) and evidence ingestion / the inbound webhook (open question 4, deferred).
...
- **Still not built:** the Gatus adapter and outward push, evidence ingestion, per-subject `integration_health` projection for fleet tools (blocked on `external_resources` / `resource_links`, not on the table), and any `/infrastructure` surface.
