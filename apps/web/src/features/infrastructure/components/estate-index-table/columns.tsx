...
/**
 * `/infrastructure/estate`'s index columns (Rule N2) — one row per
 * infrastructure-category connection, whether or not its provider has a
 * shipped estate page yet (Rule N2: "every infrastructure-category
 * connection", not only the implemented ones — a future wave's provider
 * shows up here with an inactive "Open" cell rather than not existing).
 * Read-only throughout: write-policy tier renders as a badge, never the
 * editable control `/settings/connections` owns.
 */
export function getEstateIndexColumns({
  writePolicies
}: {
